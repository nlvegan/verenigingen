...
describe("Dutch Business Logic Validation - Compliance and Standards", () => {
  ...
  describe("Dutch Banking Standards and IBAN Validation", () => {
    it("should validate Dutch IBANs against all major Dutch banks", () => {
      ...
      dutchBanks.forEach((bank) => {
        ...
        if (bank.valid) {
          ...
          cy.get('input[data-fieldname="iban"]').should(
            "have.class",
            "is-valid",
          );
    ...
    it("should validate SEPA creditor identifier format compliance", () => {
      ...
      creditorIds.forEach((cred) => {
        cy.get('input[data-fieldname="creditor_identifier"]')
          .clear()
          .type(cred.id);
        ...
        if (cred.valid) {
          cy.get('input[data-fieldname="creditor_identifier"]').should(
            "have.class",
            "is-valid",
          );
          cy.get(".creditor-validation-status").should(
            "contain",
            "Valid Dutch creditor ID",
          );
          cy.get(".sepa-compliance-indicator").should(
            "have.class",
            "compliant",
          );
        } else {
  ...
  describe("Dutch Naming Conventions and Cultural Standards", () => {
    it("should handle Dutch tussenvoegsel (name particles) correctly", () => {
      ...
      dutchNames.forEach((name) => {
        // Fill name fields
        cy.get('input[data-fieldname="first_name"]')
          .clear()
          .type(name.first_name);
        cy.get('input[data-fieldname="tussenvoegsel"]')
          .clear()
          .type(name.tussenvoegsel);
        cy.get('input[data-fieldname="last_name"]')
          .clear()
          .type(name.last_name);
        ...
        cy.get('[data-fieldname="full_name"]').should(
          "have.value",
          name.expected_display,
        );

        // Test sorting name generation (for alphabetical lists)
        cy.get('[data-fieldname="sort_name"]').should(
          "have.value",
          name.expected_sort,
        );

        // Test address formatting
        cy.get(".address-display").should(
          "contain",
          `Dhr./Mevr. ${name.expected_display}`,
        );
      });
    });
    ...
    it("should validate Dutch roepnaam (calling name) conventions", () => {
      ...
      roepnaamTests.forEach((test) => {
        ...
        cy.get(".display-name-preview").should(
          "contain",
          test.expected_display,
        );
  ...
  describe("Dutch Postal Code and Geographic Validation", () => {
    it("should validate Dutch postal code format and geographic logic", () => {
      ...
      postalCodeTests.forEach((test) => {
        ...
        if (test.valid) {
          ...
          cy.get('input[data-fieldname="postal_code"]').should(
            "have.class",
            "is-valid",
          );
          cy.get('[data-fieldname="city"]').should("have.value", test.city);
          cy.get('[data-fieldname="state"]').should(
            "have.value",
            test.province,
          );
          ...
          cy.get(".postal-validation-status").should(
            "contain",
            "Valid Dutch postal code",
          );
    ...
    it("should handle special Dutch postal code cases and territories", () => {
      ...
      specialCases.forEach((testCase) => {
        cy.get('input[data-fieldname="postal_code"]')
          .clear()
          .type(testCase.code);
        cy.get('input[data-fieldname="postal_code"]').blur();
        cy.wait(1000);
        ...
        cy.get('[data-fieldname="city"]').should(
          "have.value",
          testCase.expected_city,
        );
  ...
  describe("Dutch Association Governance and Legal Compliance", () => {
    it("should validate Dutch association membership age requirements", () => {
      ...
      ageTests.forEach((test) => {
        cy.get('input[data-fieldname="birth_date"]')
          .clear()
          .type(test.birth_date);
        cy.get('input[data-fieldname="birth_date"]').blur();
        cy.wait(500);

        // Test JavaScript age calculation and status determination
        cy.get(".membership-status-indicator").should(
          "contain",
        ...
        if (test.volunteer_eligible) {
          cy.get(".volunteer-eligibility").should(
            "contain",
            "Eligible for volunteer activities",
          );
          cy.get('button[data-label="Create Volunteer Profile"]').should(
            "not.be.disabled",
          );
        } else {
          cy.get(".volunteer-eligibility").should(
            "contain",
            "Too young for volunteer activities",
          );
          cy.get('button[data-label="Create Volunteer Profile"]').should(
            "be.disabled",
          );
        ...
        if (test.special_considerations) {
          cy.get(".special-member-notes").should(
            "contain",
            test.special_considerations,
          );
        }
      });
    });
    ...
    it("should validate Dutch GDPR and privacy compliance", () => {
      ...
      cy.get('input[data-fieldname="newsletter_consent"]').should(
        "not.be.checked",
      );
      cy.get('input[data-fieldname="data_processing_consent"]').should(
        "be.checked",
      ); // Required
      cy.get('input[data-fieldname="marketing_consent"]').should(
        "not.be.checked",
      );
      ...
      cy.get(".gdpr-validation-error").should(
        "contain",
  ...
  describe("Dutch Financial and Tax Compliance", () => {
    it("should validate Dutch tax number (BSN) format when provided", () => {
      ...
      bsnTests.forEach((test) => {
        if (cy.get('input[data-fieldname="tax_number"]').should("exist")) {
          ...
          if (test.valid) {
            cy.get('input[data-fieldname="tax_number"]').should(
              "have.class",
              "is-valid",
            );
    ...
    it("should handle Dutch membership fee calculation and VAT rules", () => {
      ...
      cy.get(".vat-calculation").should(
        "contain",
        "VAT-exempt (association membership)",
      );
      ...
      cy.get(".donation-vat-status").should(
        "contain",
        "VAT-exempt (charitable donation)",
      );
  ...
  describe("Integration Test - Complete Dutch Member Registration", () => {
    it("should complete full Dutch member registration with all validations", () => {
      ...
      cy.get(".dutch-name-validation").should(
        "contain",
        "Valid Dutch name format",
      );
      cy.get(".postal-code-validation").should(
        "contain",
        "Valid Amsterdam postal code",
      );
      ...
      cy.get(".sepa-compliance-status").should(
        "contain",
        "Dutch SEPA compliant",
      );
      cy.get(".mandate-validation").should(
        "contain",
        "Valid for Dutch direct debit",
      );
      ...
      cy.get(".member-payment-status").should(
        "contain",
        "SEPA Direct Debit Active",
      );
      cy.get(".dutch-compliance-indicator").should(
        "have.class",
        "fully-compliant",
      );
    });
