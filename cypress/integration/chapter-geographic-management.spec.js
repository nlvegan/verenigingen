...
describe("Chapter Geographic Management - JavaScript Controller Tests", () => {
  ...
  describe("Chapter Creation and Geographic Configuration", () => {
    it("should create new chapter with postal code validation via JavaScript", () => {
      ...
      const chapterData = {
        ...
        description:
          "Chapter serving Amsterdam Noord district and surrounding areas",
        city: "Amsterdam",
      ...
      postalRanges.forEach((range, index) => {
        ...
        cy.get(
          `[data-idx="${index}"] input[data-fieldname="postal_code_from"]`,
        ).type(range.from);
        cy.get(
          `[data-idx="${index}"] input[data-fieldname="postal_code_to"]`,
        ).type(range.to);
        cy.get(
          `[data-idx="${index}"] input[data-fieldname="range_description"]`,
        ).type(range.description);

        // Test JavaScript range validation
        cy.get(`[data-idx="${index}"] .range-validation-status`).should(
          "contain",
          "Valid range",
        );
      });
      ...
      cy.get('[data-fieldname="total_postal_codes"]').should(
        "have.value",
        "30",
      );
    });
    ...
    it("should detect postal code overlaps with existing chapters", () => {
      ...
      cy.get('[data-idx="0"] input[data-fieldname="postal_code_from"]').type(
        "1010",
      );
      cy.get('[data-idx="0"] input[data-fieldname="postal_code_to"]').type(
        "1025",
      );
  ...
  describe("Member Assignment and Geographic Logic", () => {
    it("should automatically assign members to chapters based on postal codes", () => {
      ...
      cy.get(".assignment-details").should(
        "contain",
        "3511 LX → Utrecht Central Chapter",
      );
      cy.get(".assignment-details").should(
        "contain",
        "3545 CH → Utrecht Central Chapter",
      );

      // Test chapter member count updates
      cy.get('[data-fieldname="active_members_count"]').should(
        "have.value",
        "2",
      );
    ...
    it("should handle members outside chapter coverage areas", () => {
      ...
      cy.get(".exclusion-reason").should(
        "contain",
        "2500 GA not in chapter range",
      );

      // Test suggestion for uncovered members
      cy.get(".coverage-suggestions").should("be.visible");
      cy.get(".suggested-chapter").should(
        "contain",
        "Consider creating Den Haag chapter",
      );
    });
  });
  ...
  describe("Board Member Management and Governance", () => {
    it("should manage chapter board members with role validation", () => {
      ...
      cy.get('[data-idx="0"] input[data-fieldname="member"]').type(
        "Board President",
      );
      cy.wait(500);
      cy.get(".awesomplete li").first().click();

      cy.get('[data-idx="0"] select[data-fieldname="role"]').select(
        "President",
      );
      cy.get('[data-idx="0"] input[data-fieldname="start_date"]').type(
        "2025-01-01",
      );
      cy.get('[data-idx="0"] input[data-fieldname="end_date"]').type(
        "2026-12-31",
      );

      // Test JavaScript role validation
      cy.get('[data-idx="0"] .role-validation-status').should(
        "contain",
        "Valid assignment",
      );
      ...
      // Add secretary
      cy.get('button[data-label="Add Board Member"]').click();
      cy.get('[data-idx="1"] input[data-fieldname="member"]').type(
        "Board Secretary",
      );
      cy.wait(500);
      cy.get(".awesomplete li").first().click();

      cy.get('[data-idx="1"] select[data-fieldname="role"]').select(
        "Secretary",
      );
      cy.get('[data-idx="1"] input[data-fieldname="start_date"]').type(
        "2025-01-01",
      );
      cy.get('[data-idx="1"] input[data-fieldname="end_date"]').type(
        "2026-12-31",
      );

      // Test role conflict detection (try to add another president)
      cy.get('button[data-label="Add Board Member"]').click();
      cy.get('[data-idx="2"] input[data-fieldname="member"]').type(
        "Board President",
      );
      cy.wait(500);
      cy.get(".awesomplete li").first().click();

      cy.get('[data-idx="2"] select[data-fieldname="role"]').select(
        "President",
      );

      // Verify JavaScript conflict detection
      cy.get('[data-idx="2"] .role-conflict-warning').should("be.visible");
      cy.get('[data-idx="2"] .role-conflict-warning').should(
        "contain",
        "President role already assigned",
      );
    ...
    it("should handle board member term management and transitions", () => {
      ...
      cy.get(".term-expiration-warning").should(
        "contain",
        "President term expires in",
      );
      ...
      cy.get('.incoming-member input[data-fieldname="member"]').type(
        "New President",
      );
      cy.get('.incoming-member select[data-fieldname="role"]').select(
        "President",
      );
      cy.get('.incoming-member input[data-fieldname="start_date"]').type(
        "2025-08-16",
      );
      cy.get('.incoming-member input[data-fieldname="end_date"]').type(
        "2027-08-15",
      );

      // Test JavaScript transition validation
      cy.get(".transition-validation").should("be.visible");
      cy.get(".transition-status").should(
        "contain",
      ...
      cy.get(".board-history-timeline").should(
        "contain",
  ...
  describe("Chapter JavaScript Controller Integration Tests", () => {
    it("should test Chapter DocType refresh event handlers and module loading", () => {
      ...
      cy.get('button[data-label="Configure Postal Codes"]').should(
        "be.visible",
      );
      cy.get('button[data-label="Assign Eligible Members"]').should(
        "be.visible",
      );
    ...
    it("should test postal code field validation and geographic calculations", () => {
      ...
      cy.get('[data-idx="0"] .range-validation-status').should(
        "contain",
        "Invalid range",
      );
      cy.get('[data-idx="0"] .validation-error').should(
        "contain",
        "From code must be less than to code",
      );

      // Test valid range
      cy.get('[data-idx="0"] input[data-fieldname="postal_code_to"]')
        .clear()
        .type("1060");
      cy.get('[data-idx="0"] .range-validation-status').should(
        "contain",
      ...
      cy.get('button[data-label="Add Postal Range"]').click();
      cy.get('[data-idx="1"] input[data-fieldname="postal_code_from"]').type(
        "1055",
      );
      cy.get('[data-idx="1"] input[data-fieldname="postal_code_to"]').type(
        "1065",
      );
