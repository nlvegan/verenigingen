  ...
  describe("Membership Form Controller Tests", () => {
    ...
    it("should test membership creation workflow", () => {
      cy.createTestMemberWithFinancialSetup().then((member) => {
        ...
        cy.fill_frappe_field("membership_type", "Regular", {
          fieldtype: "Select",
        });
  ...
  describe("Membership Period and Duration Tests", () => {
    it("should test membership period validation and calculation", () => {
      cy.createTestMemberWithFinancialSetup().then((member) => {
        ...
        cy.fill_frappe_field("membership_type", "Annual", {
          fieldtype: "Select",
        });
  ...
  describe("Membership Type and Tier Management Tests", () => {
    it("should test membership type-specific features and benefits", () => {
      cy.createTestMemberWithFinancialSetup().then((member) => {
        ...
        cy.wait_for_member_data();
        cy.fill_frappe_field("start_date", "2025-01-01", { fieldtype: "Date" });
        cy.fill_frappe_field("membership_type", "Premium", {
          fieldtype: "Select",
        });
        ...
        cy.execute_business_workflow(
          () => {
            cy.window().then((win) => {
              ...
              if (
                frm.doc.membership_type === "Premium" &&
                frm.fields_dict.premium_benefits
              ) {
    ...
    it("should test membership upgrade and downgrade workflows", () => {
      cy.createTestMemberWithFinancialSetup().then((member) => {
        ...
        cy.wait_for_member_data();
        cy.fill_frappe_field("start_date", "2025-01-01", { fieldtype: "Date" });
        cy.fill_frappe_field("membership_type", "Basic", {
          fieldtype: "Select",
        });
        ...
        cy.fill_frappe_field("membership_type", "Professional", {
          fieldtype: "Select",
        });
  ...
  describe("Financial Integration and Billing Tests", () => {
    it("should test invoice generation and payment integration", () => {
      cy.createTestMemberWithFinancialSetup().then((member) => {
        ...
        cy.fill_frappe_field("start_date", "2025-01-01", { fieldtype: "Date" });
        cy.fill_frappe_field("end_date", "2025-12-31", { fieldtype: "Date" });
        cy.fill_frappe_field("membership_fee", "150.00", {
          fieldtype: "Currency",
        });
        ...
        cy.execute_business_workflow(
          () => {
            cy.window().then((win) => {
              ...
              cy.contains("button", "View Active Dues Schedule").should(
                "exist",
              );
    ...
    it("should test payment installment and schedule management", () => {
      cy.createTestMemberWithFinancialSetup().then((member) => {
        ...
        cy.wait_for_member_data();
        cy.fill_frappe_field("start_date", "2025-01-01", { fieldtype: "Date" });
        cy.fill_frappe_field("membership_fee", "300.00", {
          fieldtype: "Currency",
        });
        cy.fill_frappe_field("payment_plan", "Quarterly", {
          fieldtype: "Select",
        });
  ...
  describe("Membership Status and Lifecycle Tests", () => {
          ...
          cy.execute_business_workflow(
            () => {
              cy.window().then((win) => {
                ...
                if (
                  status === "Suspended" &&
                  frm.fields_dict.suspension_reason
                ) {
                ...
                if (
                  status === "Cancelled" &&
                  frm.fields_dict.cancellation_processing
                ) {
    ...
    it("should test membership cancellation and refund processing", () => {
      cy.createTestMemberWithFinancialSetup().then((member) => {
        ...
        cy.fill_frappe_field("cancellation_date", "2025-06-15", {
          fieldtype: "Date",
        });
        cy.fill_frappe_field("cancellation_reason", "Member Request", {
          fieldtype: "Select",
        });
  ...
  describe("Integration with Member Management Tests", () => {
    ...
    it("should test customer and billing integration", () => {
      cy.createTestMemberWithFinancialSetup().then((member) => {
        ...
        cy.wait_for_member_data();
        cy.fill_frappe_field("start_date", "2025-01-01", { fieldtype: "Date" });
        cy.fill_frappe_field("billing_frequency", "Monthly", {
          fieldtype: "Select",
        });
  ...
  describe("Reporting and Analytics Integration Tests", () => {
    it("should test membership analytics and reporting data", () => {
      cy.createTestMemberWithFinancialSetup().then((member) => {
        ...
        cy.fill_frappe_field("membership_fee", "250.00", {
          fieldtype: "Currency",
        });
        cy.fill_frappe_field("status", "Active", { fieldtype: "Select" });
        ...
        cy.execute_business_workflow(
          () => {
            cy.window().then((win) => {
              ...
              expect(frm.doc.membership_fee).to.equal(250.0);
              ...
              cy.log(
                "Membership properly structured for comprehensive reporting",
              );
            });
            return true;
          },
