...
Cypress.Commands.add(
	...
	(condition, timeoutType = 'fieldInput', errorMessage = 'Condition not met') => {
...
Cypress.Commands.add('click_custom_button', (buttonLabel) => {
	cy.get(`button[data-label="${buttonLabel}"]`).should('be.visible').should('not.be.disabled').click();
	cy.wait(1000);
});
...
Cypress.Commands.add('fill_frappe_field', (fieldname, value, options = {}) => {
	...
	if (fieldtype === 'Link') {
		...
		cy.get(`${fieldSelector} input, ${fieldSelector} textarea`).first().clear().type(value);
	}
