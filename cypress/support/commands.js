...
Cypress.Commands.add("wait_with_config", (timeoutType) => {
  const timeout =
    CypressConfig.timeouts[timeoutType] || CypressConfig.timeouts.fieldInput;
  cy.wait(timeout);
});
 ...
 */
Cypress.Commands.add(
  "wait_for_field_ready",
  (fieldname, expectedValue = null) => {
    return cy.waitUntil(
      () => {
        return cy
 ...
 */
Cypress.Commands.add(
  "wait_for_link_field_populated",
  (fieldname, expectedValue) => {
    return cy.waitUntil(
      () => {
        return cy
          .get(`[data-fieldname="${fieldname}"] input`)
          .should("have.value", expectedValue)
          .then(() => true);
      },
      {
...
Cypress.Commands.add(
  ...
  (workflow, fallbackFn = null, workflowName = "Unknown Workflow") => {
    return cy.window().then((win) => {
      return cy.wrap(null).then(() => {
        try {
          ...
          cy.screenshot(
            `workflow-failure-${workflowName.replace(/\s+/g, "-").toLowerCase()}`,
          );
 ...
 */
Cypress.Commands.add(
  "execute_sepa_operation",
  (sepaOperation, operationType = "SEPA Operation") => {
 ...
 */
Cypress.Commands.add(
  "execute_form_operation",
  (formOperation, operationType = "Form Operation") => {
    return cy.execute_business_workflow(
...
Cypress.Commands.add(
  ...
  (fieldname, invalidValue, validValue, expectedError) => {
    // Test invalid value
    cy.fill_field(fieldname, invalidValue);
    cy.get(`[data-fieldname="${fieldname}"] input`).should(
      "have.class",
      "is-invalid",
    );
    if (expectedError) {
...
Cypress.Commands.add(
  "test_dutch_validation",
  (fieldname, testValue, expectedResult) => {
...
Cypress.Commands.add("fill_frappe_field", (fieldname, value, options = {}) => {
  ...
  if (fieldtype === "Link") {
    ...
    cy.get(`${fieldSelector} input`).clear().type(value);
...
Cypress.Commands.add(
  "test_custom_button",
  (buttonLabel, expectedAction = null) => {
...
Cypress.Commands.add(
  ...
  (fieldname, invalidValue, validValue) => {
    // Test invalid value
    ...
    cy.get(`[data-fieldname="${fieldname}"]`).should(
      "not.have.class",
      "has-error",
    );
  },
...
Cypress.Commands.add(
  "test_dynamic_ui_update",
  (triggerField, triggerValue, expectedChange) => {
    ...
    if (expectedChange.fieldVisible) {
      cy.get(`[data-fieldname="${expectedChange.fieldVisible}"]`).should(
        "be.visible",
      );
    }
    if (expectedChange.fieldHidden) {
      cy.get(`[data-fieldname="${expectedChange.fieldHidden}"]`).should(
        "not.be.visible",
      );
    }
    if (expectedChange.buttonAppears) {
      cy.get(`button:contains("${expectedChange.buttonAppears}")`).should(
        "be.visible",
      );
    }
  },
