...
def generate_monitoring_test_data():
    ...
    for i in range(5):
        ...
        print(
            f"  ✓ Resource check {i+1}: CPU={metrics.get('cpu_percent')}%, Memory={metrics.get('memory_percent')}%"
        )
...
def cleanup_test_data():
    ...
    return {"status": "success", "cleaned": {"alerts": len(test_alerts), "audit_logs": len(test_audits)}}


if __name__ == "__main__":
    print("Run via bench execute:")
    print(
        "  Generate: bench --site dev.veganisme.net execute verenigingen.scripts.testing.monitoring.generate_test_data.generate_monitoring_test_data"
    )
    print(
        "  Cleanup: bench --site dev.veganisme.net execute verenigingen.scripts.testing.monitoring.generate_test_data.cleanup_test_data"
    )
