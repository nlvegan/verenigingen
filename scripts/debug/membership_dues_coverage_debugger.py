...
Usage:
    bench --site dev.veganisme.net execute scripts.debug.membership_dues_coverage_debugger.run_full_debug
"""

import frappe
import json
from datetime import datetime, timedelta
from frappe.utils import getdate, today, date_diff, add_days, flt
    ...
    try:
        ...
        for i, member in enumerate(sample_members):
            ...
            if member.customer:
                ...
                if dues_schedule:
                    results.append(f"  - Dues: {dues_schedule.billing_frequency} @ €{dues_schedule.dues_rate}")
        ...
        if not has_start or not has_end:
            ...
            if coverage_count == 0:
                ...
                for inv in sample_coverage:
                    results.append(f"  - {inv.name}: {inv.custom_coverage_start_date} to {inv.custom_coverage_end_date}")
...
def generate_solution_recommendations():
    ...
    if not has_start or not has_end:
        ...
        results.append("")
        results.append("Required Actions:")
        results.append("1. Create custom fields in Sales Invoice DocType:")
        results.append("   - custom_coverage_start_date (Date)")
        results.append("   - custom_coverage_end_date (Date)")
        results.append("")
        results.append("2. Update existing invoices to populate coverage dates")
        results.append("3. Modify invoice creation process to set coverage dates")
        results.append("")
        
        # Generate field creation code
        results.append("Field Creation Code:")
        results.append("```python")
        results.append("# Add these custom fields to Sales Invoice")
        results.append("custom_fields = {")  
        results.append('    "Sales Invoice": [')
        results.append('        {')
        results.append('            "fieldname": "custom_coverage_start_date",')
        results.append('            "fieldtype": "Date",')
        results.append('            "label": "Coverage Start Date",')
        results.append('            "description": "Start date of membership period covered by this invoice"')
        results.append('        },')
        results.append('        {')
        results.append('            "fieldname": "custom_coverage_end_date",')
        results.append('            "fieldtype": "Date",')
        results.append('            "label": "Coverage End Date",')
        results.append('            "description": "End date of membership period covered by this invoice"')
        results.append('        }')
        results.append('    ]')
        results.append('}')
        results.append("```")
        ...
        if coverage_count == 0:
            ...
            results.append("")
            results.append("Required Actions:")
            results.append("1. Create script to populate coverage dates for existing invoices")
            results.append("2. Update invoice creation process to set coverage dates")
            results.append("3. Create validation to ensure coverage dates are set")
            results.append("")
        else:
    ...
    results.append("ALTERNATIVE SOLUTION: Use Alternative Date Fields")
    results.append("=" * 50)
    results.append("If custom coverage fields cannot be added immediately,")
    results.append("modify the report to use existing date fields:")
    results.append("")
    results.append("Option 1: Use posting_date for coverage calculations")
    results.append("Option 2: Use due_date and calculate coverage periods")
    results.append("Option 3: Infer coverage from membership dues schedule")
    results.append("")
    
    # Code fix recommendations
    results.append("IMMEDIATE CODE FIX")
    results.append("=" * 20)
    results.append("File: membership_dues_coverage_analysis.py")
    results.append("Line: 365-400 (get_member_invoices_with_coverage function)")
    results.append("")
    results.append("Current issue: Function queries for non-existent fields")
    results.append("Immediate fix: Add field existence check and fallback logic")
    results.append("")
    
    results.append("TESTING RECOMMENDATIONS")
    results.append("=" * 25)
    results.append("1. Test with debug_coverage_fields() function")
    results.append("2. Create sample invoices with coverage dates")
    results.append("3. Run report on single member first")
    results.append("4. Validate membership period calculations")
    results.append("5. Check error logs for additional issues")
    results.append("")
...
def quick_coverage_test(member_name=None):
    ...
    try:
        ...
        from verenigingen.verenigingen.report.membership_dues_coverage_analysis.membership_dues_coverage_analysis import get_membership_periods
        periods = get_membership_periods(member_name)
        ...
        from verenigingen.verenigingen.report.membership_dues_coverage_analysis.membership_dues_coverage_analysis import calculate_coverage_timeline
        ...
        if gaps:
            results.append(f"\nGaps ({len(gaps)}):")
            for gap in gaps:
                results.append(f"  - {gap['gap_start']} to {gap['gap_end']}: {gap['gap_days']} days ({gap['gap_type']})")
        else:
            results.append(f"\nNo gaps found")
            ...
            try:
                ...
                if billing_frequency == 'Monthly':
                    ...
                    if coverage_start.month == 12:
                        coverage_end = coverage_start.replace(year=coverage_start.year + 1, month=1, day=1) - timedelta(days=1)
                    else:
                        coverage_end = coverage_start.replace(month=coverage_start.month + 1, day=1) - timedelta(days=1)
                    ...
                    if quarter == 4:
                        ...
                        coverage_end = coverage_start.replace(month=quarter_start_month + 3, day=1) - timedelta(days=1)
                ...
                # Update the invoice
                frappe.db.set_value('Sales Invoice', invoice_data.name, {
...
if __name__ == "__main__":
    ...
    print("bench --site dev.veganisme.net execute scripts.debug.membership_dues_coverage_debugger.run_full_debug")