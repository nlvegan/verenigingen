#!/bin/bash
# Cleanup script for development - removes duplicates without archiving

echo "Cleaning up monitoring folder (development mode)..."

# Remove duplicate scripts
echo "Removing duplicate scripts..."
rm -f zabbix_integration.py
rm -f zabbix_v7_advanced.py

# Remove old cleanup script
rm -f cleanup_monitoring.sh

# Update the structure documentation
cat > MONITORING_STRUCTURE.md << 'EOF'
...
echo ""
echo "Summary:"
echo "- Single implementation: zabbix_integration_enhanced.py"
echo "- Simple import in: vereinigingen/monitoring/zabbix_integration.py"
echo "- All duplicates removed"
echo "- Templates and docs preserved"