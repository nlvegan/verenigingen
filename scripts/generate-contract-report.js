...
} = require("../verenigingen/tests/setup/api-contract-simple");
const fs = require("fs");
...
class ContractReportGenerator {
  ...
  generateComprehensiveReport() {
    ...
    methods.forEach((method) => {
      ...
      try {
        ...
        const result = this.tester.validateFrappeCall({
          method,
          args: testData,
        });
    ...
    console.log("\n📄 Reports Generated:");
    console.log(
      `   - JSON Report: ${this.reportDir}/comprehensive-contract-report.json`,
    );
  ...
  assessCriticality(method) {
    if (method.includes("payment") || method.includes("mollie")) {
      return "high";
    }
    if (method.includes("sepa") || method.includes("mandate")) {
      return "high";
    }
    if (method.includes("member") || method.includes("financial")) {
      return "medium";
    }
    return "low";
  }
...
`;
  ...
  generateCoverageMatrix(contractResults) {
    const matrix = {
      domains: {
        financial: contractResults.filter(
          (r) => r.method.includes("payment") || r.method.includes("mollie"),
        ),
        sepa: contractResults.filter(
          (r) => r.method.includes("sepa") || r.method.includes("mandate"),
        ),
        membership: contractResults.filter(
          (r) => r.method.includes("member") && !r.method.includes("payment"),
        ),
        chapter: contractResults.filter((r) => r.method.includes("chapter")),
        donation: contractResults.filter((r) => r.method.includes("donation")),
      },
...
try {
  ...
  console.log(
    `   Contracts: ${result.passedContracts}/${result.totalContracts}`,
  );
