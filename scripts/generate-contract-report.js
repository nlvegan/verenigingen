...
class ContractReportGenerator {
	...
	analyzeCriticalAPIs(contractResults) {
		...
		const criticalAPIs = contractResults.filter((result) =>
			criticalKeywords.some((keyword) => result.method.toLowerCase().includes(keyword))
		);
...
`;
	...
	generateCoverageMatrix(contractResults) {
		const matrix = {
			domains: {
				financial: contractResults.filter((r) => r.method.includes('payment') || r.method.includes('mollie')),
				sepa: contractResults.filter((r) => r.method.includes('sepa') || r.method.includes('mandate')),
				membership: contractResults.filter((r) => r.method.includes('member') && !r.method.includes('payment')),
