...
Ultra-refined field validation system that addresses the core false positive patterns:
1. Child table field access (board_member.is_active)
2. Improved DocType inference through context analysis
3. Better handling of recursive references
4. Enhanced settings field detection
"""
...
class FinalFieldValidator:
    """Final refined field validator with minimal false positives"""
    ...
    def __init__(self, app_path: str):
        ...
        print(f"✅ Loaded {len(self.doctypes)} DocTypes for final comprehensive validation")
    ...
    def _guess_doctype_from_context(self, content: str, node: ast.Attribute, obj_name: str) -> Optional[str]:
        ...
        line_start = max(0, node.lineno - 10)
    ...
    def _validate_with_regex_enhanced(self, content: str, file_path: Path) -> List[ValidationIssue]:
        ...
        # Pattern for obj.field access - more restrictive
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for match in re.finditer(pattern, line):
                ...
                if self.is_excluded_pattern(obj_name, field_name, line):
                    continue
                
                # Skip settings/config field access
                if self.is_settings_or_config_field(obj_name, field_name, line):
                    continue
                
                # Check for child table access
                child_doctype = self.is_child_table_access(obj_name, field_name, line, lines, line_num)
                if child_doctype:
                    continue  # Let AST handle this more accurately
                    
                # Only flag if it looks like a DocType field access
                if self._looks_like_doctype_field_access(obj_name, field_name, line, lines, line_num):
                    violations.append(ValidationIssue(
                        file=str(file_path.relative_to(self.app_path)),
                        line=line_num,
                        field=field_name,
                        doctype="Unknown",
                        reference=f"{obj_name}.{field_name}",
                        message=f"Potential field access '{field_name}' needs verification",
                        context=stripped_line,
                        confidence="medium",
                        issue_type="potential_field_access",
                        suggested_fix="Verify if this is a valid DocType field"
                    ))
                    
        return violations
        
    def _looks_like_doctype_field_access(self, obj_name: str, field_name: str, 
                                        line: str, lines: List[str], line_num: int) -> bool:
        """Check if this looks like a DocType field access using regex"""
        
        # Skip if it has method call indicators
        if f'{field_name}(' in line:
            return False
            
        # Skip if it's an assignment
        if f'{obj_name}.{field_name} =' in line:
            return False
            
        # Only flag if there are strong DocType indicators in the surrounding context
        ...
        return any(indicator in context for indicator in doctype_indicators)
