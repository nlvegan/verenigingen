#!/usr/bin/env python3
"""
AST Field Analyzer with Enhanced Detection for Hook Files

This is the production version of the AST Field Analyzer that extends the core
implementation with additional features:
- File path-based DocType inference for hook files
- QueryBuilder DocType() variable tracking
- Lambda parameter tracking to reduce false positives

The core implementation is in ast_field_analyzer_core.py.
"""
...
# Import from local core module (no longer importing from archived folder)
try:
    from .ast_field_analyzer_core import (
        ASTFieldAnalyzer as CoreAnalyzer,
        ValidationContext,
        ValidationIssue,
        ConfidenceLevel,
    )
...
class ASTFieldAnalyzer(CoreAnalyzer):
    """Enhanced analyzer with file path inference and QueryBuilder support"""
    ...
    def detect_doctype_with_modern_logic(self, node, source_lines, file_context):
        """Override to add QueryBuilder DocType recognition and file path inference"""
...
def main():
    ...
    print("AST Field Analyzer - Enhanced Version with Hook File Support")
    print("="*60)
