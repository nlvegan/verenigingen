"""The AST predicates the non-resumable-error ratchets share.
...
``non_resumable_errors.py`` (a DIFFERENT module -- it builds real exception instances and
needs frappe) exists next to the original copy of this file.

MOVED HERE FROM ``verenigingen/tests/support/non_resumable_ast.py`` (#1189). This module is
stdlib-only (``ast`` only) and always has been; it moved so
``scripts/validation/savepoint_rollback_validator.py`` -- the standalone, pre-commit-invokable
entry point for the #561 ratchet -- can import it without pulling in ``frappe``. Importing
ANYTHING under ``verenigingen.tests.*`` runs ``verenigingen/tests/__init__.py``, which does
``import frappe`` at module scope; the stdlib-only CI job that runs pre-commit-style validators
installs no such thing (measured: `import verenigingen.tests.support.non_resumable_ast` raises
``ModuleNotFoundError: No module named 'frappe'`` from a bare ``python3`` with no bench on the
path). ``verenigingen/tests/support/non_resumable_ast.py`` is now a thin re-export of this file,
kept so its two other bench-side consumers do not need to change their imports.
