...
- `bulk_process_member_payments(payment_ids, docstatus=0, payment_modes=None, create_bank_transactions=None)` — the shared ~140-LOC body. Enqueues **this module's** `process_payment_batch_job` (single canonical batch worker; replaces the two per-page copies).
- `process_payment_batch_job(batch_num, payment_ids, docstatus, payment_modes, tracking_id)` — the shared batch worker (one copy).
...
- `sanitize_csv_field(value: str) -> str` — the CSV-injection-prevention helper (one copy). `mollie_bulk_payment_creation.py` and `mollie_subscription_recreation.py` import it and drop their local copies.
...
3. **Batch-job enqueue path changes** (internal): both pages now enqueue the single `...bulk_payment_admin_service.process_payment_batch_job` instead of their per-page copies. Edge case: a background job already queued under an old per-page path at deploy time would still resolve (the old function still exists until this ships) — no in-flight-job migration needed because the old paths are removed only when the wrappers are, in the same deploy; document that a redeploy should drain the Mollie bulk queue first if any job is in flight (operational note, not a code change).
4. **Access checks NOT unified** — kept page-local (identical role lists today; page-semantic).

## Test strategy

- Locate existing tests exercising these functions (e.g. `test_bulk_payment_checker`, any `test_*mollie*` covering the admin endpoints) via grep for the function names + the two page dotted paths. Repoint tests that call the moved logic to the service, OR keep them calling the page endpoints (which still exist as wrappers) — prefer keeping endpoint-level tests as integration coverage and add service-level unit tests for the consolidated functions.
...
- **Shrink:** `templates/pages/mollie_payments_debug.py`, `templates/pages/mollie_payment_processing.py` (5 endpoints each → thin wrappers; move `_retrieve_global_payments_with_orphans` out of processing)
