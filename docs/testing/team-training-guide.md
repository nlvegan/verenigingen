...
expect(apiCall).toMatchAPIContract(
...
// 3. Create Test Suite
describe(
  "Your DocType Controller (Comprehensive Tests)",
...
const customWorkflowTests = {
  "Approval Workflow": (getControllerTest) => {
    const workflowBuilder = createDomainTestBuilder(
      getControllerTest(),
      "workflow",
    );
...
const config = {
  controllerPath:
    "/home/frappe/frappe-bench/apps/verenigingen/verenigingen/verenigingen/doctype/your_doctype/your_doctype.js",
};
```
...
const apiCall = {
  ...
  payment_amount: 25.0, // Number, not string
  payment_method: "SEPA Direct Debit",
};
```
...
it("should integrate with payment processing workflow", () => {
  ...
  let paymentCallback;
  controllerTest.mockForm.add_custom_button.mockImplementation(
    (text, callback) => {
...
_This guide represents the current state of our testing infrastructure as of January 2025. It will be updated as the system evolves._
