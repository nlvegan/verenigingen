...
- **E1 (submit batch to bank) — CONFIRMED stub, NUANCED (likely intentional).** `process_batch` (`direct_debit_batch.py:302`, wired to a "Submit to Bank" button via `direct_debit_batch.js:141`) sets status→"Submitted" + logs success but has **no bank-API call** ("placeholder"). HOWEVER the SEPA `pain.008` file **is** generated first, and SEPA direct debit is commonly submitted by **manually uploading that file** to the bank portal — so this is plausibly the intended manual workflow with only the *automated-API* submission unbuilt, NOT "collections never happen." **Confirm the org's actual bank-submission process** before treating as a defect.
