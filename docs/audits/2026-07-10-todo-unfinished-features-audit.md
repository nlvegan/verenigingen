...
- **C6 (audit_trail compliance score) — CONFIRMED.** `_check_audit_trail_gaps` (`analytics_engine.py:1211`) returns a hardcoded `score: 85.0` (self-labelled "Stub") and is called at `:341`, feeding `_calculate_overall_compliance_score` (`:1223`) → a **fabricated number in the overall compliance score**. Impact = wherever that analytics compliance report is surfaced.
