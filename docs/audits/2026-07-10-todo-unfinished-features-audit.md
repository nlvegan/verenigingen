...
- **D (require_permission bypass) — CONFIRMED → ✅ FIXED (PR #144).** The always-`True` `require_permission` (singular, `error_handling.py:594`) had **zero callers** (the grep hits are `require_permissions`, plural — a different, real function). **Fix:** removed the dead stub (decommissioned, not implemented — like C2/C3); the real alternatives (`require_permission_decorator`, `validate_admin_access`, `frappe.has_permission`) are untouched.
- **A1 — CONFIRMED visible → ✅ FIXED (PR #146).** "Generate PDF" (`periodic_donation_agreement.js:92`→`generate_agreement_pdf`) was a "Phase 3" msgprint. **Fix:** real agreement-document PDF (render → PDF → attach as private File, idempotent), reusing the C1 machinery via a shared `_attach_generated_pdf()` helper. Distinct from C1 (C1 = tax receipt; A1 = signable agreement doc).
- **A4 — CONFIRMED visible → ✅ FIXED (PR #147).** "Set subscription" (`mollie_member_reconciliation.html:448`→`setMemberSubscription`, "coming soon") lacked the member id. **Fix (JS-only):** the button is inside the member's card, so thread `member.member_id` into `renderSubscriptions` → call the existing tested `update_member_mollie_fields` endpoint (mirrors the sibling `applyQuickFix`). No new backend.
- **A5 — CONFIRMED visible → ✅ FIXED (PR #145).** The dashboard "Expense Approvals (N)" header (`chapter_dashboard.html:236`) was fed by a hardcoded `pending_expenses = []` (`chapter_dashboard.py:543`) → always 0. **Fix:** `get_pending_actions` now calls a new `get_pending_expense_approvals` (real Draft `Expense Claim` query, same filter as the existing stat card) and the template renders the list; button gated on `can_approve_expenses`.
- **A3 — CONFIRMED visible → ✅ FIXED Phase 1 (PR #148); Pay.nl = Phase 2 (TODO).** "Make Payment" (`payment_plans.html:343`→`showPaymentForm`, "coming soon"). **Fix (Phase 1, Mollie only):** new `/payment_plan_pay` page + `Payment Plan Payment` intent doctype handed to the unchanged `MollieGateway`; the Mollie webhook gains a dispatch (before donation classification) that finalizes the installment via the existing `PaymentPlan.process_payment()` **only after Mollie confirms payment**. Design/plan under `docs/superpowers/{specs,plans}/2026-07-11-payment-plan-make-payment-mollie*`. **Remaining: Phase 2 = Pay.nl / ING Checkout** (separate integration, not in the shared `PaymentHook`; own spec/PR). Deployment note: the Mollie `webhook_user` must hold the FINANCIAL tier (defaults to Administrator, which passes).
- **A2 (donate ANBI section) — CONFIRMED, lower severity → ACCEPTED as-is (no fix).** `{% if false %}` (`donate.html:517`) with a "Disabling ... for now" comment — a cleanly *hidden* section, not a broken/visible control. No user sees a dead button; left as-is (delete the dead block only if the disable is permanent).
