...
class TestVolunteerExpenseMockElimination(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        self.test_member = self.create_test_member(
            first_name="Volunteer", last_name="Expense", email="volunteer.expense@test.example.com"
        )

        # Create real volunteer linked to member
        self.test_volunteer = self.create_test_volunteer(
            member=self.test_member.name, volunteer_name="Test Volunteer Expense"
        )

        # Create real chapter for membership validation
        self.test_chapter = self.create_chapter(introduction="Test chapter for volunteer expense validation")
    ...
    def test_real_chapter_membership_validation_logic(self):
        ...
        try:
            chapter_member = self.create_test_chapter_member(
                member=self.test_member.name, chapter=self.test_chapter.name, status="Active"
            )
    ...
    def test_real_expense_amount_limits_validation(self):
        ...
        for test_case in amount_test_cases:
            try:
                ...
                if test_case["valid"]:
                    print(
                        f"✅ Real amount validation: €{test_case['amount']:.2f} - {test_case['description']}"
                    )
        ...
        try:
            ...
            if actual_total and actual_total[0][0]:
                ...
                if abs(actual_amount - total_expected) < 0.01:
                    print(
                        f"✅ Real expense reporting consistency: Expected €{total_expected:.2f}, Got €{actual_amount:.2f}"
                    )
                else:
                    print(
                        f"ℹ️  Real reporting includes other expenses: Expected €{total_expected:.2f}, Got €{actual_amount:.2f}"
                    )
    ...
    def test_real_performance_volunteer_expense_operations(self):
        ...
        self.assertLess(
            elapsed, 10.0, f"Real expense operations should complete in <10s, took {elapsed:.3f}s"
        )
...
print(
    "Run with: bench --site dev.veganisme.net run-tests --module verenigingen.tests.unit.test_volunteer_expense_mock_elimination"
)
