"""
Debug, admin, and test endpoints for chapter dashboard.
Extracted from chapter_dashboard_api.py to separate debug/admin tooling from production API.
"""
    ...
    try:
        ...
        try:
            ...
            results["tests"]["background_jobs"] = {
                "status": "Passed" if enqueue_available else "Warning",
        ...
        # Summary
        passed_tests = sum(1 for test in results["tests"].values() if "Passed" in test["status"])
        ...
        results["summary"] = {
            ...
            "success_rate": f"{(passed_tests / total_tests) * 100:.1f}%",
            "status": "Framework Ready" if passed_tests == total_tests else "Some Issues Found",
        }
