...
def get_member_age_distribution_chart() -> OperationResult[Dict[str, Any]]:
    """
    Get member age distribution data formatted for dashboard charts
    Can be called directly from JavaScript

    Returns:
        OperationResult with chart data containing labels, datasets, and type
    """
    try:
        ...
        chart_data = {"labels": labels, "datasets": [{"name": _("Members"), "values": values}], "type": "bar"}

        return OperationResult.ok(
            chart_data, message=_("Member age distribution chart data retrieved successfully")
        )

    except Exception as e:
        frappe.log_error(
            f"Error generating member age distribution chart: {str(e)}\n{traceback.format_exc()}",
            "Member Age Distribution Chart Failed",
        )
        return OperationResult.fail(
            _("Failed to generate member age distribution chart"),
            errors=[str(e)],
            context={"operation": "get_member_age_distribution_chart"},
        )
