...
def check_sepa_indexes() -> OperationResult[Dict[str, Any]]:
    ...
    try:
                ...
                if not index_data:
                    ...
                    if len(indexed_columns) != len(expected_columns):
                        ...
                        results["found"].append(
                            {"table": table, "index": index_name, "columns": indexed_columns}
                        )
        ...
        if data["success"]:
            message = _("All {0} SEPA database indexes are properly configured").format(len(indexes_to_check))
        else:
            message = _("SEPA index check completed: {0} found, {1} missing, {2} incomplete").format(
                data["found_count"], data["missing_count"], data["incomplete_count"]
            )
        ...
        frappe.log_error(
            title=_("SEPA Index Check Failed"),
            message=traceback.format_exc(),
        )
        return OperationResult.fail(
            _("Failed to check SEPA database indexes"),
            errors=[str(e)],
