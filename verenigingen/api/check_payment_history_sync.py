...
def check_invoice_payment_history_sync() -> OperationResult[Dict[str, Any]]:
    ...
    try:
        ...
        results["summary"] = {
            ...
            "sync_rate": (
                f"{len(members_with_history)}/{len(members_with_invoices)}"
        ...
        message = _("Payment history sync check completed for {0} invoices").format(len(todays_invoices))
        if members_without_history:
            message = _("Payment history sync check completed. {0} members missing history").format(
                len(members_without_history)
            )
        ...
        frappe.log_error(
            title=_("Payment History Sync Check Failed"),
            message=traceback.format_exc(),
        )
        return OperationResult.fail(
            _("Failed to check payment history sync"),
            errors=[str(e)],
    ...
    try:
        ...
        todays_invoices = frappe.get_all(
            ...
            filters={
                "posting_date": today_date,
                "docstatus": 1,
                "creation": [">=", f"{today_date} 07:20:00"],
            },
        ...
        message = _("Manually synced payment history for {0} invoices").format(results["synced"])
        if results["errors"]:
            message = _("Synced {0} invoices with {1} errors").format(
                results["synced"], len(results["errors"])
            )
        ...
        frappe.log_error(
            title=_("Manual Payment History Sync Failed"),
            message=traceback.format_exc(),
        )
        return OperationResult.fail(
            _("Failed to manually sync payment history"),
            errors=[str(e)],
            context={"traceback": traceback.format_exc()},
        )
