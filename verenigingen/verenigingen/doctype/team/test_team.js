...
QUnit.test("test: Team Creation", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      assert.equal(
        cur_frm.doc.team_name,
        "Test JS Team",
        "Team name set correctly",
      );
      assert.equal(
        cur_frm.doc.team_type,
        "Working Group",
...
QUnit.test("test: Team Responsibilities", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      assert.equal(
        cur_frm.doc.key_responsibilities.length,
        2,
        "Two responsibilities added",
      );

      // Check if both responsibilities are in the list
      let responsibilities = cur_frm.doc.key_responsibilities.map(
        (r) => r.responsibility,
      );
      assert.ok(
        responsibilities.includes("Test Task 1") &&
          responsibilities.includes("Test Task 2"),
...
QUnit.test("test: Team Member Status Change", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      assert.equal(
        cur_frm.doc.team_members[0].status,
        "Inactive",
        "Status changed to inactive",
      );
      assert.equal(
        cur_frm.doc.team_members[0].is_active,
        0,
        "Is active flag synchronized with status",
      );
    },
    () => done(),
  ]);
