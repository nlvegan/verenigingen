...
QUnit.test("test: Volunteer Creation", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      assert.equal(
        cur_frm.doc.volunteer_name,
        "Test Volunteer JS",
        "Volunteer name set correctly",
      );
      assert.equal(cur_frm.doc.status, "Active", "Status set correctly");
      assert.equal(
        cur_frm.doc.commitment_level,
        "Regular (Monthly)",
        "Commitment level set correctly",
      );
    },
    () => done(),
  ]);
...
QUnit.test("test: Volunteer Skills", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      assert.equal(
        cur_frm.doc.skills_and_qualifications.length,
        2,
        "Two skills added",
      );
      assert.equal(
  ...
  frappe.run_serially([
    ...
    () => {
      ...
      assert.equal(
        cur_frm.doc.name,
        volunteer_name,
        "Volunteer form loaded correctly",
      );
      assert.equal(
        cur_frm.doc.status,
        "Active",
        "Volunteer still has active status",
      );

      // We can only visually confirm the activity appears in the aggregated view
      assert.ok(
        true,
        "Activity should appear in aggregated assignments (visual confirmation needed)",
      );
    },
    () => done(),
  ]);
  ...
  frappe.run_serially([
    ...
    (volunteer) => {
      ...
      assert.equal(
        volunteer.member,
        member_name,
        "Member correctly linked to volunteer",
      );
      // Volunteer name should be populated from member
      assert.ok(
        volunteer.volunteer_name,
        "Volunteer name populated from member",
      );
    },
    () => done(),
  ]);
...
QUnit.test("test: Volunteer History and Assignment History", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      ...
      if (
        cur_frm.doc.assignment_history &&
        cur_frm.doc.assignment_history.length > 0
      ) {
        // Look for the completed activity
        let found = cur_frm.doc.assignment_history.some(
          (h) =>
        ...
        assert.ok(
          false,
          "Assignment history should contain completed activity",
        );
      }
    },
    () => done(),
