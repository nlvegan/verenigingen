...
class Donation(Document):
    ...
    def create_sales_invoice(self):
        """DEPRECATED: Sales Invoice creation removed - using Payment History child table model"""
        # This method is deprecated and does nothing
        import traceback

        stack_trace = traceback.format_stack()
        frappe.logger().error(
            f"🚨 DEPRECATED: create_sales_invoice() called for {self.name} - SOMEONE IS STILL CALLING THIS!"
        )
        frappe.logger().error(f"🚨 Call stack: {''.join(stack_trace[-5:])}")  # Last 5 stack frames
        return None
    ...
    def on_submit(self):
        ...
        # Note: Sales Invoice creation removed - using Payment History child table model instead
...
def create_donation_from_bank_transfer(donor, amount, date, bank_reference, donation_type=None):
    ...
    # Note: Donation submission removed - using Payment History child table model
