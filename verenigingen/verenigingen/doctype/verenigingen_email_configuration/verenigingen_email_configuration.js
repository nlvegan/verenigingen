...
frappe.ui.form.on('Verenigingen Email Configuration', {
	refresh(frm) {
		...
		frm.add_custom_button(
			__('Enable All'),
			() => {
				frm.doc.notification_types.forEach((row) => {
		...
		frm.add_custom_button(
			__('Disable All'),
			() => {
				frappe.confirm(__('Are you sure you want to disable all notification types?'), () => {
					frm.doc.notification_types.forEach((row) => {
...
frappe.ui.form.on('Email Notification Type', {
	notification_types_add(frm, cdt, cdn) {
...
function show_sync_dialog(frm, data) {
	...
	dialog.$wrapper.find('.new-key-checkbox').on('change', () => {
		update_button_count();
	});
