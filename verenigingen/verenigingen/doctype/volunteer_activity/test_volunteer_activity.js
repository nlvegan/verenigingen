...
QUnit.test("test: Volunteer Activity Creation", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      assert.equal(
        cur_frm.doc.volunteer,
        test_volunteer,
        "Volunteer set correctly",
      );
      assert.equal(
        cur_frm.doc.activity_type,
        "Project",
        "Activity type set correctly",
      );
      assert.equal(
        cur_frm.doc.role,
        "Project Coordinator",
  ...
  frappe.run_serially([
    ...
    () => {
      assert.equal(
        ...
        "Team",
        "Reference doctype set correctly",
      );
      assert.equal(
        cur_frm.doc.reference_name,
        "Test Team",
        "Reference name set correctly",
      );
    },
    () => done(),
  ]);
