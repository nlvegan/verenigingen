...
QUnit.test("test: Membership", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      assert.equal(
        cur_frm.doc.member_name,
        "Membership Test Member",
        "Member name should be fetched automatically",
      );
      assert.equal(
        cur_frm.doc.status,
        "Draft",
      ...
      assert.equal(
        yearDiff,
        1,
        "Renewal date should be 1 year after start date",
      );
    },
    ...
    () => {
      ...
      assert.ok(
        mandateField.df.reqd,
        "SEPA Mandate should be required for SEPA Direct Debit",
      );

      // Check if mandate section is visible
      let mandateSection = $(cur_frm.fields_dict.mandate_section.wrapper);
      assert.ok(
        mandateSection.is(":visible"),
        "Mandate section should be visible for SEPA Direct Debit",
      );
    },
    () => done(),
  ]);
