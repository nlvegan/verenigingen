...
frappe.ui.form.on('Mijnrood CSV Import', {
	refresh(frm) {
		...
		if (frm.doc.import_status === 'Completed') {
			...
			if (frm.doc.create_volunteer_records) {
				frm.add_custom_button(
					__('Retry Volunteer Creations'),
					() => {
						frappe.confirm(
							__(
								'This will attempt to create volunteer records for members that are missing them. Continue?'
							),
							() => {
								frappe.call({
			...
			if (frm.doc.create_user_accounts && frm.doc.acrs_failed > 0) {
				frm.add_custom_button(
					__('Retry Account Creations'),
					() => {
						frappe.confirm(
							__('This will retry creating user accounts for {0} failed requests. Continue?', [
								frm.doc.acrs_failed
							]),
							() => {
								frappe.call({
		...
		if (!frm.doc.csv_file) {
			...
			frm.set_intro(
				__('File selected: {0}. Click "Validate CSV" to process and validate the file.', [fileName]),
				'orange'
			);
			...
			if (frm.doc.test_mode) {
				frm.set_intro(
					__('Ready to import in test mode (no records will be created). Review preview data below.'),
					'green'
				);
			} else {
				frm.set_intro(
					__('Ready to import! This will create actual member records. Review preview data below.'),
					'orange'
				);
			}
