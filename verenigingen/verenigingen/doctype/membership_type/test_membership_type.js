...
QUnit.test("test: Membership Type", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      ...
      let periodMonthsField = cur_frm.get_field(
        "dues_schedule_period_in_months",
      );
      assert.ok(
        $(periodMonthsField.wrapper).is(":visible"),
        "Months field should be visible for Custom period",
      );
      assert.ok(
        periodMonthsField.df.reqd,
        "Months field should be required for Custom period",
      );
    },
    () => done(),
  ]);
...
QUnit.test("test: Membership Type - Default Setting", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      ...
      assert.equal(
        cur_frm.doc.default_for_new_members,
        1,
        "Second type should be set as default",
      );
    },
...
QUnit.test("test: Membership Type - Subscription Plan", function (assert) {
  ...
  frappe.run_serially([
    ...
    () => {
      ...
      let planButton = cur_frm.page.inner_toolbar.find(
        'button:contains("Create Subscription Plan")',
      );
      assert.ok(
        planButton.length,
        "Create Subscription Plan button should exist",
      );

      // Initially dues_schedule_plan should be empty
      assert.ok(
        !cur_frm.doc.dues_schedule_plan,
        "Subscription Plan field should be empty",
      );
    },
    () => done(),
  ]);
