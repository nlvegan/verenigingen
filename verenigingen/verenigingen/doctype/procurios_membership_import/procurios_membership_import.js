...
frappe.ui.form.on('Procurios Membership Import', {
	refresh(frm) {
			...
			frm.add_custom_button(__('Validate CSV'), () => {
				frappe.call({
					method:
						...
						'procurios_membership_import.validate_import_file',
	...
	onload(frm) {
		...
		if (frm.is_new()) {
			frm.dashboard.set_headline(
				__(
					'Imports memberships from the Procurios membership-export CSV. ' +
						"Only rows whose Debiteur Id matches an existing Member's procurios_id are imported."
				)
			);
		}
	}
