frappe.ui.form.on('MijnRood Sync Settings', {
	...
	refresh(frm) {
		...
		const needs_first_time_setup = !frm.doc.status_mapping?.length || !frm.doc.role_mapping?.length;
		if (needs_first_time_setup) {
			frm.add_custom_button(__('First-time setup'), () => {
				frappe.confirm(
					__(
						'This creates the default teams (Landelijk Beheer + Secretariaat) and populates any empty status/role mapping tables with recommended defaults. Existing data is preserved. Continue?'
					),
					() => {
						frappe.call({
				...
				callback(r) {
					if (r.message && r.message.success) {
						frappe.msgprint({
							...
							indicator: 'green',
							message: __('Connected to MijnRood database. Found {0} rows in admin_member.', [
								r.message.row_count
							])
						});
					} else {
		...
		frm.add_custom_button(
			__('Fetch from MijnRood'),
			() => {
				frappe.confirm(
					__(
						'Fetch membership statuses from MijnRood? Existing admin configuration (Membership Type, Termination Type) will be preserved.'
					),
					() => {
						frappe.call({
					...
					fields: [
						{
							...
							description: __(
								'Paste the full private key including -----BEGIN ... KEY----- and -----END ... KEY----- lines'
							),
							options: 'Text'
						}
					],
		...
		frm.add_custom_button(
			...
			() => {
				frappe.confirm(
					__(
						'Fetch document folders from MijnRood? This will populate the folder mapping table with root-level folders.'
					),
					() => {
						frappe.call({
		...
		if (frm.doc.document_folder_mapping && frm.doc.document_folder_mapping.length) {
			frm.add_custom_button(
				...
				() => {
					frappe.confirm(
						__(
							'Auto-classify folder mappings? This will infer document types and chapters from folder names. Only blank rows will be updated — manual edits are preserved.'
						),
						() => {
							frappe.call({
		...
		frm.add_custom_button(
			...
			() => {
				frappe.confirm(
					__(
						'Start importing documents from MijnRood? Files will be downloaded via SFTP and Organization Document records created. This runs as a background job.'
					),
					() => {
						frappe.call({
		...
		if (!frm._import_progress_bound) {
			...
			frappe.realtime.on('document_import_progress', (data) => {
		...
		frm.add_custom_button(__('Sync Now'), () => {
			frappe.confirm(__('Start an immediate sync with MijnRood? This will run in the background.'), () => {
				frappe.call({
