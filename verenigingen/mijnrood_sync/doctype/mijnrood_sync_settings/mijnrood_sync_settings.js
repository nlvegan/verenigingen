frappe.ui.form.on('MijnRood Sync Settings', {
	...
	refresh(frm) {
		...
		frm.add_custom_button(__('Fetch from MijnRood'), () => {
			frappe.confirm(
		...
		if (!frm.doc.status_mapping || !frm.doc.status_mapping.length) {
			frm.add_custom_button(__('Load Defaults'), () => {
				frappe.call({
		...
		frm.add_custom_button(__('Set SSH Key'), () => {
		...
		frm.add_custom_button(__('Set SSH Key Passphrase'), () => {
			open_secret_dialog(
		...
		frm.add_custom_button(__('Set SSH Password'), () => {
			open_secret_dialog(
				'Set SSH Password',
				'Password',
		...
		frm.add_custom_button(__('Diagnose SSH Auth'), () => {
		...
		frm.add_custom_button(__('Fetch Folders'), () => {
			frappe.confirm(
		...
		if (frm.doc.document_folder_mapping && frm.doc.document_folder_mapping.length) {
			frm.add_custom_button(__('Auto-Classify'), () => {
				frappe.confirm(
		...
		frm.add_custom_button(__('Import Documents'), () => {
			frappe.confirm(
