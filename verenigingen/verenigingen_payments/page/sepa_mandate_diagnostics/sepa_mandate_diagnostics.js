...
function load_diagnostics(page) {
	...
	$(page.body).html(
		`<div class="text-center margin-top"><p class="text-muted">${__('Loading diagnostics...')}</p></div>`
	);
...
function fix_all_for_issue(issue_type) {
	frappe.confirm(__('This will fix all members with this issue type. Continue?'), () => {
