...
frappe.ui.form.on('Direct Debit Batch', {
	...
	onload(frm) {
		...
		// Set query filters
		frm.set_query('invoice', 'invoices', () => {
			return {
				filters: {
...
function submit_to_bank_dialog(frm) {
	frappe.confirm(__('Are you sure you want to submit this batch to the bank? This action cannot be undone.'), () => {
		frappe.call({
...
function process_returns_dialog(frm) {
	const dialog = new frappe.ui.Dialog({
		...
		primary_action(values) {
			frappe.call({
				method: 'verenigingen.verenigingen_payments.utils.bank_transaction_reconciliation.process_sepa_return_file',
				args: {
				...
				callback(r) {
					...
					const data = unwrapOperationResult(r.message);
					if (data) {
						frappe.msgprint(__('Processed {0} of {1} returns', [data.processed, data.total]));
						frm.reload_doc();
						dialog.hide();
					}
				}
		...
		primary_action(values) {
			...
			frappe.call({
				...
				callback(r) {
					...
					if (!r.exc && r.message) {
						frm.reload_doc();
						frappe.msgprint(__('Successfully processed {0} payments', [r.message]));
						dialog.hide();
					}
				}
			});
...
function setup_realtime_updates(frm) {
	if (!frm.is_new()) {
		frappe.realtime.on(`dd_batch_update_${frm.doc.name}`, (data) => {
			if (data.message) {
				frappe.show_alert({
