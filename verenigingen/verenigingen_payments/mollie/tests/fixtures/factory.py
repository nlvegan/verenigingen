...
class MollieTestDataFactory(EnhancedTestDataFactory):
    ...
    def create_mollie_subscription_scenario(
        self, member_name: str = None, amount: float = None
    ) -> Dict[str, Any]:
    ...
    def create_webhook_payload(
        self, subscription_id: str = None, payment_id: str = None, status: str = "paid"
    ) -> Dict[str, Any]:
        ...
        payload = {
            ...
            "payment": {
                ...
                "mode": "test",
                "status": status,
                "amount": {"value": f"{self.generate_realistic_amount():.2f}", "currency": "EUR"},
                "createdAt": datetime.now().isoformat(),
    ...
    def create_payment_test_data(self, count: int = 5) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            scenario["webhook_payload"] = self.create_webhook_payload(
                payment_id=scenario["payment_id"], status=scenario["status"]
            )
    ...
    def create_performance_test_data(self, webhook_count: int = 25) -> List[Dict[str, Any]]:
        ...
        for i in range(min(10, webhook_count)):
            member = self.create_mollie_test_member(first_name=f"PerfTest{i:03d}", last_name="Member")
...
class MollieTestCase(EnhancedTestCase):
    ...
    def assertPaymentProcessed(self, payment_id: str, expected_amount: float):
        ...
        self.assertGreater(len(payment_entries), 0, f"No payment entry found for payment ID {payment_id}")

        payment_entry = payment_entries[0]
        self.assertEqual(
            flt(payment_entry["paid_amount"]), expected_amount, f"Payment amount mismatch for {payment_id}"
        )
        self.assertEqual(
            payment_entry["docstatus"], 1, f"Payment entry {payment_entry['name']} should be submitted"
        )
    ...
    def assertWebhookProcessingTime(self, processing_time_ms: int, max_time_ms: int = 1000):
        ...
        self.assertLessEqual(
            processing_time_ms,
            max_time_ms,
            f"Webhook processing took {processing_time_ms}ms, " f"which exceeds maximum of {max_time_ms}ms",
        )

    def simulate_webhook_load(
        self, webhook_count: int = 25, time_window_seconds: int = 1
    ) -> List[Dict[str, Any]]:
