...
from verenigingen.verenigingen_payments.mollie.services.generic_webhook_service import GenericWebhookService
...
class TestMollieWebhookSecurity(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        self.test_member = self.create_test_member(
            first_name="Webhook", last_name="Security", email="webhook.security@test.example.com"
        )
    ...
    def test_webhook_signature_validation_comprehensive(self):
        ...
        webhook_data = self.create_test_mollie_webhook_data(
            webhook_type="payment.paid", payment_id=self.test_payment_id, amount=100.0
        )
        ...
        self.assertTrue(results["valid_signature"], "Valid HMAC signature should pass validation")
        ...
        self.assertTrue(results["payload_integrity"], "Payload should maintain integrity")
        self.assertTrue(results["timing_attack_resistance"], "Should use constant-time comparison")
    ...
    def test_webhook_signature_timing_attack_resistance(self):
        ...
        """
        webhook_data = self.create_test_mollie_webhook_data(
            webhook_type="payment.paid", payment_id=self.test_payment_id
        )
        ...
        self.assertLess(max_variance, avg_time * 2, "Signature validation timing should be consistent")

        print(
            f"✅ Timing attack resistance validated (avg: {avg_time*1000:.2f}ms, max variance: {max_variance*1000:.2f}ms)"
        )
    ...
    def test_webhook_security_audit_logging(self):
        ...
        with self.assertQueryCount(self.webhook_performance_baselines["audit_logging"]):
            ...
            invalid_result = self.processor.process_payment_webhook(
                webhook_payload=payload_json, signature="invalid_signature_for_testing"
            )
    ...
    def test_webhook_payload_validation_security(self):
        ...
        validation_error = self.processor._validate_webhook_payload(valid_webhook["webhook_payload"])
    ...
    def test_webhook_idempotency_protection(self):
        ...
        payment_entries = frappe.get_all("Payment Entry", filters={"reference_no": self.test_payment_id})

        # Should not create duplicate entries
        self.assertLessEqual(len(payment_entries), 1, "Should not create duplicate Payment Entries")
    ...
    def test_webhook_performance_baselines(self):
        ...
        webhook_data = self.create_test_mollie_webhook_data(
            webhook_type="payment.paid", payment_id=f"perf_test_{frappe.generate_hash()[:8]}"
        )
