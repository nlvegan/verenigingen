...
class TestMollieRefundChargebackWebhookProcessing(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        self.test_donation.append("payments", {
    ...
    def test_refund_webhook_processing(self):
        ...
        self.assertTrue(refund_history, f"Donation payment history should include refund. Found {len(donation.payments)} total payments")
    ...
    def test_payment_history_idempotency(self):
        ...
        for idx, p in enumerate(donation.payments):
            print(f"  Entry {idx + 1}: PE={p.payment_entry}, amount={p.amount}, mollie_id={p.mollie_payment_id}")

        # Verify we have exactly 2 refund entries (not duplicates of the first one)
        refund_entries_1_after = [p for p in donation.payments if p.mollie_payment_id == "re_history_test_001"]
        refund_entries_2 = [p for p in donation.payments if p.mollie_payment_id == "re_history_test_002"]

        self.assertEqual(len(refund_entries_1_after), 1,
                        "First refund should still appear exactly once (no duplicates)")
        self.assertEqual(len(refund_entries_2), 1,
                        "Second refund should appear exactly once")
        ...
        refund_entries_1_final = [p for p in donation.payments if p.mollie_payment_id == "re_history_test_001"]
        self.assertEqual(len(refund_entries_1_final), 1,
