...
class TestSecureOperationsCoverage(EnhancedTestCase):
    ...
    # ==================================================================
    # secure_batch_operation
    # ==================================================================
    def test_batch_operation_all_succeed(self):
        from verenigingen.utils.secure_operations import secure_batch_operation

        ops = []
        for i in range(2):
            d = frappe.new_doc("ToDo")
            d.description = f"batch todo {i}"
            ops.append({"operation": "create", "doc": d, "allow_system_user": False})

        results = secure_batch_operation(ops, justification="batch create of todos")
        self.assertEqual(len(results), 2)
        self.assertTrue(all(r.success for r in results), [r.errors for r in results])

    def test_batch_operation_handles_malformed_op(self):
        from verenigingen.utils.secure_operations import secure_batch_operation

        # Missing "doc" key -> KeyError inside the loop, captured as an error result.
        results = secure_batch_operation(
            [{"operation": "create"}], justification="malformed batch op", fail_fast=True
        )
    ...
    # ==================================================================
    def test_execute_critical_unconfigured_operation_runs(self):
        from verenigingen.utils.secure_operations import execute_critical_operation

        doc = frappe.new_doc("ToDo")
        doc.description = "unconfigured critical op"
        result = execute_critical_operation(
            operation_name="totally_unconfigured_op_xyz",
            operation="create",
            doc=doc,
            justification="Runs through the unconfigured fallback branch",
        )
        self.assertTrue(result.success, f"errors: {result.errors}")
        self.assertIsNotNone(result.doc_name)

    def test_execute_critical_blocks_on_critical_business_violation(self):
        from verenigingen.utils.secure_operations import (
            execute_critical_operation,
            get_critical_operations_registry,
        )

        reg = get_critical_operations_registry()
        reg.operation_configs["inj_critical_viol"] = {
            "security_level": "critical",
            "business_rules": {"enabled": True, "amount_threshold": 100},
        ...
        try:
            doc = frappe.new_doc("ToDo")
            doc.description = "should never be inserted"
            result = execute_critical_operation(
                operation_name="inj_critical_viol",
                operation="create",
                doc=doc,
            ...
            self.assertFalse(result.success)
            self.assertTrue(any("Critical business rule" in e for e in result.errors))
            # Operation must NOT have executed.
            self.assertIsNone(result.doc_name)
        finally:
            reg.operation_configs.pop("inj_critical_viol", None)

    def test_execute_critical_requires_justification(self):
        from verenigingen.utils.secure_operations import (
            execute_critical_operation,
            get_critical_operations_registry,
        )
        ...
        reg = get_critical_operations_registry()
        reg.operation_configs["inj_needs_just"] = {
            "security_level": "high",
            "business_rules": {"enabled": False},
            "requires_justification": True,
        }
        try:
            doc = frappe.new_doc("ToDo")
            doc.description = "no justification"
            result = execute_critical_operation(
                operation_name="inj_needs_just",
                operation="create",
                doc=doc,
                justification=None,
            )
            self.assertFalse(result.success)
            self.assertTrue(any("Justification is required" in e for e in result.errors))
            self.assertIsNone(result.doc_name)
        finally:
            reg.operation_configs.pop("inj_needs_just", None)

    def test_execute_critical_configured_success(self):
        from verenigingen.utils.secure_operations import (
            execute_critical_operation,
            get_critical_operations_registry,
        )
        ...
        reg = get_critical_operations_registry()
        reg.operation_configs["inj_ok_op"] = {
            "security_level": "high",
            "business_rules": {"enabled": True, "amount_threshold": 1000},
        ...
        try:
            doc = frappe.new_doc("ToDo")
            doc.description = "configured critical success"
            result = execute_critical_operation(
                operation_name="inj_ok_op",
                operation="create",
                doc=doc,
                justification="Configured critical op that should succeed",
                amount=10,  # under threshold
            )
            self.assertTrue(result.success, f"errors: {result.errors}")
            self.assertIsNotNone(result.doc_name)
        finally:
            reg.operation_configs.pop("inj_ok_op", None)

    # ==================================================================
    # execute_bulk_member_operation convenience wrapper
    # ==================================================================
    def test_execute_bulk_member_operation(self):
        from verenigingen.utils.secure_operations import execute_bulk_member_operation

        op_data = []
        for i in range(2):
            d = frappe.new_doc("ToDo")
            d.description = f"bulk member todo {i}"
            op_data.append({"doc": d, "operation": "create"})

        results = execute_bulk_member_operation(op_data, justification="bulk member op test")
        self.assertEqual(len(results), 2)
        self.assertTrue(all(r.success for r in results), [r.errors for r in results])

    # ==================================================================
    # SecureOperationResult.doc typo-guard
    # ==================================================================
