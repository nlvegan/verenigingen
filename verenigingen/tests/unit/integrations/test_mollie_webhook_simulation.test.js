...
class MockWebhookHandler {
	...
	getStats() {
		return {
			...
			success_rate:
				this.errors.length === 0 ? 100 : (this.processedEvents.length / this.receivedWebhooks.length) * 100
		};
	}
...
describe('Mollie Webhook Simulation with MSW', () => {
	...
	describe('Subscription Webhook Simulation', () => {
		it('should process active subscription webhook', async () => {
			const payload = MollieWebhookPayloadGenerator.generateSubscriptionWebhook('sub_webhook_test_001', 'active');
			const body = JSON.stringify(payload);
			const signature = signatureGenerator.generateSignature(body);
	...
	describe('Chargeback Webhook Simulation', () => {
		it('should process chargeback webhook', async () => {
			...
			expect(result.event.journalEntry.accounts[0].debit_in_account_currency).toBe(25.0);
		});
	});
	...
	describe('Webhook Security and Validation', () => {
		it('should reject webhook with invalid signature', async () => {
			const payload = MollieWebhookPayloadGenerator.generatePaymentWebhook('tr_invalid_sig_001', 'paid', '25.00');
	...
	describe('Webhook Performance and Reliability', () => {
		...
		it('should support webhook replay for failed processing', async () => {
			const payload = MollieWebhookPayloadGenerator.generatePaymentWebhook('tr_replay_test', 'paid', '25.00');
			const body = JSON.stringify(payload);
			const signature = signatureGenerator.generateSignature(body);
	...
	describe('End-to-End Webhook Scenarios', () => {
		it('should simulate complete payment lifecycle via webhooks', async () => {
			...
			const openPayload = MollieWebhookPayloadGenerator.generatePaymentWebhook(paymentId, 'open', '30.00');
			...
			const paidPayload = MollieWebhookPayloadGenerator.generatePaymentWebhook(paymentId, 'paid', '30.00');
		...
		it('should simulate subscription lifecycle with multiple events', async () => {
			...
			const activePayload = MollieWebhookPayloadGenerator.generateSubscriptionWebhook(subscriptionId, 'active');
