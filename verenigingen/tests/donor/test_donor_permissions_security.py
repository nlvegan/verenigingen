...
class TestDonorPermissionsSecurity(VereningingenTestCase):
    ...
    def test_permission_bypass_attempt_with_document_manipulation(self):
        ...
        # Create a malicious document-like object. Capture the test's donor and
        # member names here (inside __init__ ``self`` refers to MaliciousDoc, not
        # the test case).
        donor_name = self.test_donor.name
        member_name = self.test_member.name

        class MaliciousDoc:
            def __init__(self):
                self.name = donor_name
                self.member = member_name  # Try to fake ownership
        ...
        with self.as_user("bypass_attempt@example.com"):
    ...
    def test_unauthorized_access_comprehensive(self):
        ...
        for scenario in unauthorized_scenarios:
            with self.subTest(description=scenario["description"]):
                ...
                with MockRolesContext(scenario["roles"]):
                    ...
                    self.assertFalse(
                        has_permission, f"Unauthorized access granted to: {scenario['description']}"
                    )
    ...
    def test_integration_with_frappe_orm_security(self):
        ...
        with self.as_user("orm_test@example.com"):
            # Should be able to access linked donor via ORM
            accessible_donors = frappe.get_all(
                "Donor", fields=["name", "donor_name", "member"], filters={"name": orm_donor.name}
            )
            ...
            with self.subTest(input=repr(malformed_input)):
                try:
                    ...
                    self.assertIsInstance(
                        e,
                        (ValueError, TypeError, frappe.ValidationError),
...
class TestDonorPermissionsEdgeCases(VereningingenTestCase):
    ...
    def test_concurrent_access_simulation(self):
        ...
        self.assertTrue(
            all(result for result in all_results), "Concurrent access should return consistent results"
        )
...
class TestDonorPermissionsRealWorldScenarios(VereningingenTestCase):
    ...
    def test_foppe_user_scenario_security_validation(self):
        ...
        with self.as_user("foppe@veganisme.org"):
            ...
            accessible_donors = frappe.get_all(
                "Donor", fields=["name", "donor_name"], filters={"name": foppe_donor.name}
            )
