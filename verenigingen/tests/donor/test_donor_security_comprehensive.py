...
class TestDonorSecurityComprehensive(EnhancedTestCase):
    ...
    def test_sql_injection_prevention_member_field_escaping(self):
        ...
        for name_variant in problematic_names:
            with self.subTest(member_name=name_variant):
                ...
                try:
                    # Without a role granting member-scoped access, the query is
                    # always the restrictive "1=0" - the escaping branch below
                    # never runs for this call, so assert the deterministic value.
                    query = get_donor_permission_query(test_member.email)
                    self.assertEqual(query, "1=0")

                    # Grant the member role to exercise the actual escaping path:
                    # the query embeds the escaped member docname, and must be
                    # safe to execute even though the member's name contains
                    # SQL-special characters.
                    with frappe.mock_roles(["Verenigingen Member"]):
                        scoped_query = get_donor_permission_query(test_member.email)

                    self.assertIn(f"`tabDonor`.member = {frappe.db.escape(test_member.name)}", scoped_query)
                    test_sql = f"SELECT COUNT(*) FROM `tabDonor` WHERE {scoped_query}"
                    result = frappe.db.sql(test_sql)
                    self.assertEqual(
                        result[0][0], 1, "Query should match exactly the one donor linked to this member"
                    )
    ...
    def test_enhanced_error_handling_invalid_member_links(self):
        ...
        result = has_donor_permission(self.invalid_linked_donor.name, "test_member_user@example.com")
    ...
    def test_permission_enforcement_member_access_own_donor(self):
        ...
        with self.as_user("test_member_user@example.com"):
            ...
            result = has_donor_permission(self.linked_donor.name, "test_member_user@example.com")
    ...
    def test_permission_enforcement_member_denied_other_donor(self):
        ...
        with self.as_user("test_member_user@example.com"):
            ...
            result = has_donor_permission(self.other_linked_donor.name, "test_member_user@example.com")
    ...
    def test_permission_enforcement_admin_override_access(self):
        ...
        """
        admin_roles_to_test = ["System Manager", "Verenigingen Administrator", "Verenigingen Staff"]
        ...
        for role in admin_roles_to_test:
            with self.subTest(admin_role=role):
                ...
                result_orphaned = has_donor_permission(self.orphaned_donor.name, "Administrator")
    ...
    def test_permission_enforcement_orphaned_donor_access(self):
        ...
        result = has_donor_permission(self.orphaned_donor.name, "test_member_user@example.com")
    ...
    def test_permission_query_filtering_member_records(self):
        ...
        """
        # setUp created a real User for 'test_member_user@example.com' with the
        # Verenigingen Member role, linked to self.test_member - so the query
        # deterministically filters to that member's own records.
        query = get_donor_permission_query("test_member_user@example.com")
        self.assertEqual(
            query,
            f"(`tabDonor`.member = {frappe.db.escape(self.test_member.name)})",
        )
    ...
    def test_integration_frappe_get_all_respects_permissions(self):
        ...
        # EnhancedTestCase handles permissions automatically
        admin_donors = frappe.get_all("Donor", fields=["name", "donor_name", "member"])
        ...
        with self.as_user("test_member_user@example.com"):
            try:
                user_donors = frappe.get_list("Donor", fields=["name", "donor_name", "member"])
    ...
    def test_document_object_vs_string_parameter_handling(self):
        ...
        result_string = has_donor_permission(self.linked_donor.name, "test_member_user@example.com")

        # Test with document object parameter
        donor_doc = frappe.get_doc("Donor", self.linked_donor.name)
        result_object = has_donor_permission(donor_doc, "test_member_user@example.com")
    ...
    def test_performance_with_multiple_permission_checks(self):
        ...
        try:
            ...
            self.assertLess(execution_time, 2.0, f"Permission checks took too long: {execution_time:.2f}s")
    ...
    def tearDown(self):
        ...
        test_members = [self.test_member.name, self.other_member.name, self.memberless_user.name]
...
class TestDonorSecurityPerformance(EnhancedTestCase):
    ...
    def test_permission_query_generation_performance(self):
        ...
        test_users = ["Administrator", "Guest", "test_member@example.com", "nonexistent@user.com"]
        ...
        self.assertLess(execution_time, 0.5, f"Permission query generation too slow: {execution_time:.3f}s")
    ...
    def test_sql_injection_protection_overhead(self):
        ...
        test_member = self.create_test_member(
            first_name="Performance", last_name="Test", email="performance@example.com"
        )
        ...
        try:
            ...
            self.assertLess(execution_time, 0.1, f"SQL escaping overhead too high: {execution_time:.3f}s")
