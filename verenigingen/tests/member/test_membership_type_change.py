...
class TestMembershipTypeChange(BaseTestCase):
    ...
    def setUp(self):
        ...
        test_types = QueryBuilder.get_all_active_records(
            "Membership Type", additional_filters={"name": ["like", "TEST-%"]}
        )
        ...
        self.basic_type = self.create_membership_type(
            name="Basic Monthly", minimum_amount=10.00, description="Basic monthly membership"
        )
        ...
        self.member = self.create_test_member(
            first_name="Type", last_name="Changer", email="type.changer@test.com"
        )
        ...
        # Submitting the membership auto-creates an active dues schedule via hooks.
        # Reuse it if present; otherwise create one explicitly.
        ...
        if existing_schedule:
            ...
            self.track_doc("Membership Dues Schedule", existing_schedule)
        else:
    ...
    def test_approve_membership_type_change(self):
        ...
        new_schedule = frappe.get_value(
            "Membership Dues Schedule", {"member": self.member.name, "status": "Active"}, "name"
        )
