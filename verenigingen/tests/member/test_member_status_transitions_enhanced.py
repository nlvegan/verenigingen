...
class TestMemberStatusTransitionsEnhanced(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        self.chapter = self.factory.ensure_test_chapter(
            "Member Status Test Chapter", {"short_name": "MSTC", "country": "Netherlands"}
        )

        # Create membership types
        self.regular_type = self.factory.ensure_membership_type(
            "Regular Member",
            {"amount": 100.00, "currency": "EUR", "subscription_period": "Annual", "is_active": 1},
        )

        self.student_type = self.factory.ensure_membership_type(
            "Student Member",
            {"amount": 50.00, "currency": "EUR", "subscription_period": "Annual", "is_active": 1},
        )
    ...
    def create_chapter_membership(self, member_name: str, chapter_name: str):
        ...
        existing = frappe.get_all(
            "Chapter Member", filters={"member": member_name, "parent": chapter_name}, limit=1
        )
        if existing:
            return
        ...
        chapter.append(
            "members", {"member": member_name, "enabled": 1, "chapter_join_date": today(), "status": "Active"}
        )
        chapter.save()
    ...
    def test_active_to_suspended_transition(self):
        ...
        with self.assertQueryCount(800):
            ...
            member = self.create_test_member(first_name="Active", last_name="Member", status="Active")
    ...
    def test_suspended_to_active_transition(self):
        ...
        member = self.create_test_member(first_name="Suspended", last_name="Member", status="Suspended")
    ...
    def test_termination_requires_workflow(self):
        ...
        # Create active member using factory
        member = self.create_test_member(first_name="ToTerminate", last_name="Member", status="Active")
    ...
    def test_membership_status_lapsed_without_active_membership(self):
        """A member with no submitted membership derives membership_status = 'Lapsed'.

        Replaces a bare `pass` stub whose premise ("Terminated" transitions) referenced a
        status value that isn't in Member.status options. Instead pin the real derivation
        in member_status_service.update_member_membership_status: with no docstatus=1
        Membership, membership_status falls to "Lapsed".
        """
        ...
        # No Membership was submitted for this member -> derivation yields Lapsed on save.
        member.save()
        member.reload()
    ...
    def test_pending_status_transitions(self):
        ...
        member = self.create_test_member(first_name="Pending", last_name="Member", status="Pending")
    ...
    def test_concurrent_status_changes(self):
        ...
        member = self.create_test_member(
            first_name="Concurrent", last_name="Member", status="Active", chapter=self.chapter.name
        )
    ...
    def test_existing_member_defaults_application_status_to_approved(self):
        """An existing member with an empty application_status is normalized to Approved.

        Replaces a bare `pass` stub. set_member_application_status_defaults distinguishes
        new applications (default "Pending") from existing members (default "Approved",
        member_status_service.py line ~53). Blanking application_status on an already-saved
        member and re-saving must yield "Approved", not "Pending".
        """
        member = self.create_test_member(
            first_name="DefaultApp",
            last_name="Member",
            status="Active",
        )

        # Member already exists (not new); blank its application_status and re-save.
    ...
    def test_membership_status_active_with_submitted_membership(self):
        """A member with a current submitted membership derives membership_status = 'Active'.

        Replaces a bare `pass` stub. update_member_membership_status marks the member
        Active when a docstatus=1 Membership exists whose renewal_date is not in the past.
        """
        member = self.create_test_member(
            first_name="HasMembership",
            last_name="Member",
            status="Active",
        )

        # Submitted, current membership -> derivation yields Active on next save.
        # Submitting the membership touches the member row, so reload before re-saving
        # to avoid a TimestampMismatchError.
        self.create_test_membership(member.name, self.regular_type.name)
    ...
    def test_member_status_membership_cascade(self):
        ...
        # Create member using factory
        member = self.create_test_member(first_name="Cascade", last_name="Test", status="Active")
    ...
    def test_status_change_audit_trail(self):
        ...
        # Create member using factory
        member = self.create_test_member(
            first_name="Audit", last_name="Trail", status="Active", chapter=self.chapter.name
        )
        ...
        try:
            ...
            if frappe.db.exists("DocType", "Communication History"):
                audit_entries = frappe.get_all(
                    ...
                    filters={"comm_type": "Email", "comm_description": ["like", f"%{member.name}%"]},
                )
                if audit_entries:
    ...
    def test_payment_status_business_rules(self):
        ...
        # Create member using factory
        member = self.create_test_member(
            first_name="Payment", last_name="Rules", status="Active", chapter=self.chapter.name
        )
    ...
    def test_chapter_transfer_status_rules(self):
        ...
        # Create member using factory
        member = self.create_test_member(
            first_name="Transfer", last_name="Test", status="Active", chapter=self.chapter.name
        )
    ...
    def test_permission_context_for_status_changes(self):
        ...
        member = self.create_test_member(
            first_name="Permission", last_name="Test", status="Active", chapter=self.chapter.name
        )
