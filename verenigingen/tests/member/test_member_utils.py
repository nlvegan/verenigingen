...
class TestMemberUtils(EnhancedTestCase):
    ...
    def _add_member_to_chapter(self, member_name, chapter_name, status="Active"):
        ...
        chapter_doc.append("members", {"member": member_name, "status": status, "enabled": 1})
        chapter_doc.save()
        return chapter_doc
    ...
    def test_get_member_name_for_user_fallback_user_field(self):
        ...
        member = self.create_test_member(first_name="Legacy", last_name="User", birth_date="1985-01-01")
    ...
    def test_get_volunteer_for_member_propagates_database_error(self):
        """The volunteer step of the same resolver chain must propagate too.

        Fixing only get_member_name_for_user left this hole open: chapter_utils
        calls this one line later, so a database error here still degraded to
        "no volunteer" -> "no board positions" -> "no chapter access", exactly the
        silent denial the sibling fix removed.

        It is worse than a denial in one place. Volunteer.create_volunteer_from_member
        uses this as its duplicate guard, where None means "no volunteer exists yet" -
        so a swallowed error creates a SECOND Volunteer for a member who already has
        one. mijnrood_sync's volunteer_sync_service guards its inserts the same way.
        """
        outage = frappe.db.OperationalError("simulated database outage")

        with patch.object(frappe.db, "get_value", side_effect=outage):
            with self.assertRaises(frappe.db.OperationalError):
                get_volunteer_for_member("Assoc-Member-does-not-matter")
    ...
    def test_get_active_membership_for_member_success(self):
        ...
        membership = self.create_test_membership(
            member_name=self.member.name, membership_type_name=membership_type.name
        )
    ...
    def test_validate_member_ownership_failure(self):
        ...
        other_member = self.create_test_member(
            first_name="Other", last_name="Member", email="other@test.com", birth_date="1985-01-01"
        )
...
class TestDuesScheduleUtilities(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        self.membership_type = self.create_test_membership_type(membership_type_name="Standard", amount=50.0)
    ...
    def _cleanup_membership_schedules(self):
        ...
        """
        existing_schedules = frappe.get_all(
            "Membership Dues Schedule", filters={"membership": self.membership.name}, pluck="name"
        )
        for schedule_name in existing_schedules:
            try:
                frappe.delete_doc("Membership Dues Schedule", schedule_name, force=True)
    ...
    def _create_dues_schedule(self, status="Active", dues_rate=50.0, is_template=0):
        ...
        if schedule.get("membership") != self.membership.name:
            frappe.db.set_value("Membership Dues Schedule", schedule.name, "membership", self.membership.name)
            schedule.reload()
    ...
    def test_get_member_dues_schedule_custom_fields(self):
        ...
        result = get_member_dues_schedule(self.member.name, fields=["name", "dues_rate", "status"])
    ...
    def test_field_validation_invalid_fields_filtered(self):
        ...
        result = get_member_dues_schedule(
            self.member.name, fields=["name", "dues_rate", "nonexistent_field", "status"]
        )
    ...
    def test_field_validation_all_invalid_fields(self):
        ...
        result = get_member_dues_schedule(self.member.name, fields=["invalid_field_1", "invalid_field_2"])
