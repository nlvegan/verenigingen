...
class TestDutchIBANValidation(EnhancedTestCase):
    ...
    def test_invalid_dutch_iban_real_validation(self):
        ...
        member = self.create_test_member(first_name="IBAN", last_name="InvalidTest")
    ...
    def test_iban_bank_code_recognition_real_logic(self):
        ...
        member = self.create_test_member(first_name="BankCode", last_name="Recognition")
...
class TestDutchNameHandling(EnhancedTestCase):
    ...
    def test_name_search_real_functionality(self):
        ...
        for first, middle, last in test_members:
            member = self.create_test_member(first_name=first, middle_name=middle, last_name=last)
        ...
        berg_members = frappe.get_all(
            "Member", filters={"last_name": ["like", "Berg%"]}, fields=["name", "full_name"]
        )
        ...
        van_berg_members = frappe.get_all(
            "Member", filters={"full_name": ["like", "%van Berg%"]}, fields=["name", "full_name"]
        )
...
class TestDutchAgeValidation(EnhancedTestCase):
    ...
    def _member_at_age(self, age):
        ...
        member.last_name = f"AgeTest{h}"
        member.birth_date = birth_date.strftime("%Y-%m-%d")
        member.email = f"age{age}.{h}@test.invalid"
        member.status = "Active"
        return member

    def test_member_age_requirement_enforced_at_model_level(self):
        """Minimum age (Verenigingen Settings.minimum_membership_age) is enforced on
        member save: under-age rejected, at/above the minimum allowed."""
        min_age = int(frappe.db.get_single_value("Verenigingen Settings", "minimum_membership_age"))

        for age in (min_age - 1, min_age - 4):
            with self.subTest(age=age, expect="rejected"):
                with self.assertRaises(frappe.ValidationError):
                    self._member_at_age(age).save()

        for age in (min_age, min_age + 1, 25, 65):
            with self.subTest(age=age, expect="allowed"):
                member = self._member_at_age(age)
                member.save()
                self.assertIsNotNone(member.name)
                self.track_doc("Member", member.name)

    def test_volunteer_age_requirement_real_validation(self):
        """At/above the minimum age a member can be created and become a volunteer."""
        min_age = int(frappe.db.get_single_value("Verenigingen Settings", "minimum_volunteer_age"))
        ...
        for age in (min_age, min_age + 1, 25, 65):
            with self.subTest(age=age):
                ...
                volunteer.volunteer_name = member.full_name
                volunteer.email = f"volunteer.{member.name}@test.invalid"
...
class TestDutchMembershipLifecycle(EnhancedTestCase):
    ...
    def test_membership_termination_real_workflow(self):
        ...
        member = self.create_test_member(first_name="Termination", last_name="TestMember", status="Active")
        ...
        termination_request.termination_reason = (
            "Testing termination workflow"  # Fixed: use correct field name
        )
...
class TestDutchDataPrivacyCompliance(EnhancedTestCase):
    ...
    def test_gdpr_data_retention_real_rules(self):
        ...
        member = self.create_test_member(first_name="GDPR", last_name="RetentionTest", status="Quit")
    ...
    def test_consent_management_real_tracking(self):
        ...
        member = self.create_test_member(first_name="Consent", last_name="Management")
...
class TestDutchFinancialCompliance(EnhancedTestCase):
    ...
    def test_sepa_compliance_real_validation(self):
        ...
        member = self.create_test_member(first_name="SEPA", last_name="Compliance")
    ...
    def test_dutch_tax_compliance_real_rules(self):
        ...
        member = self.create_test_member(first_name="Tax", last_name="Compliance")
    ...
    def test_financial_reporting_real_compliance(self):
        ...
        member = self.create_test_member(first_name="Financial", last_name="Reporting")
...
class TestDutchBusinessLogicPermissionBoundaries(EnhancedTestCase):
    ...
    def test_member_creation_permission_validation(self):
        ...
        admin_member = self.create_test_member(
            first_name="Admin", last_name="Created", email="admin.created@permission.test"
        )
        ...
        try:
            ...
            has_permission_control = any(
                keyword in error_message for keyword in ["permission", "access", "not allowed", "denied"]
            )
    ...
    def test_sepa_mandate_permission_validation(self):
        ...
        # EnhancedTestCase handles permissions: frappe.set_user(self.admin_user)
        member = self.create_test_member(
            first_name="SEPA", last_name="Permission", email="sepa.permission@test.com"
        )
    ...
    def test_sensitive_data_access_permission_validation(self):
        ...
        # EnhancedTestCase handles permissions: frappe.set_user(self.admin_user)
        member = self.create_test_member(
            first_name="Sensitive", last_name="Data", email="sensitive.data@test.com"
        )
...
class TestComplexDutchBusinessWorkflows(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        self.test_chapter = self.factory.ensure_test_chapter(
            "Complex Workflow Chapter", {"short_name": "CWC", "country": "Netherlands", "published": 1}
        )

        # Create membership type with complex business rules
        self.complex_membership_type = self.factory.ensure_membership_type(
            "Complex Member",
            {"minimum_amount": 50.0, "billing_period": "Monthly", "enforce_minimum_period": 1},
        )
    ...
    def test_complete_member_lifecycle_workflow(self):
        ...
        self.test_chapter.append(
            "members",
            {"member": pending_member.name, "enabled": 1, "status": "Active", "chapter_join_date": today()},
        )
        ...
        active_mandates = frappe.get_all(
            "SEPA Mandate", filters={"member": final_member.name, "status": "Active"}
        )
    ...
    def test_multi_chapter_membership_workflow(self):
        ...
        multi_member = self.create_test_member(
            first_name="Multi", last_name="Chapter", email="multi.chapter@workflow.test"
        )

        # Create second chapter
        second_chapter = self.factory.ensure_test_chapter(
            "Second Chapter", {"short_name": "SC2", "country": "Netherlands", "published": 1}
        )

        # Add member to first chapter
        self.test_chapter.append(
            "members",
            {"member": multi_member.name, "enabled": 1, "status": "Active", "chapter_join_date": today()},
        )
        self.test_chapter.save()

        # Add member to second chapter
        second_chapter.append(
            "members",
            {"member": multi_member.name, "enabled": 1, "status": "Active", "chapter_join_date": today()},
        )
        second_chapter.save()
        ...
        chapter_memberships = frappe.get_all(
            "Chapter Member", filters={"member": multi_member.name, "enabled": 1}, fields=["parent", "status"]
        )
    ...
    def test_payment_workflow_integration(self):
        ...
        try:
            ...
            invoice.append("items", {"item_code": "Membership Fee", "qty": 1, "rate": 50.0, "amount": 50.0})
            invoice.save()
        ...
        if invoice:
            ...
            invoices = frappe.get_all(
                "Sales Invoice", filters={"customer": customer.name}, fields=["name", "grand_total", "status"]
            )
...
class TestStandardizedErrorMessageValidation(EnhancedTestCase):
                    ...
                    if "format" in expected_error_type:
                        self.assertTrue(
                            any(
                                ...
                                for keyword in ["format", "pattern", "invalid"]
                            ),
                        ...
                        self.assertTrue(
                            any(
                                keyword in error_message.lower() for keyword in ["digit", "number", "numeric"]
                            ),
                    ...
                    if error_category == "checksum":
                        ...
                        self.assertTrue(
                            any(
                                ...
                                for keyword in ["country", "dutch", "netherlands", "checksum", "invalid"]
                            ),
    ...
    def test_membership_validation_error_messages(self):
        ...
        try:
            ...
            self.assertTrue(
                any(
                    keyword in error_message.lower() for keyword in ["membership", "type", "invalid", "exist"]
                ),
            ...
            with self.subTest(email=invalid_email):
                try:
                    ...
                    if error_category == "format":
                        self.assertTrue(
                            any(
                                keyword in error_message.lower() for keyword in ["format", "valid", "address"]
                            ),
                        ...
                        self.assertTrue(
                            any(
                                keyword in error_message.lower()
                                for keyword in ["required", "mandatory", "cannot be empty"]
                            ),
...
class TestPerformanceBenchmarkBaselines(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        self.perf_chapter = self.factory.ensure_test_chapter(
            "Performance Chapter", {"short_name": "PERF", "country": "Netherlands", "published": 1}
        )

        self.perf_membership_type = self.factory.ensure_membership_type(
            "Performance Member", {"minimum_amount": 25.0, "billing_period": "Monthly"}
        )
    ...
    def test_sepa_mandate_creation_performance_baseline(self):
        ...
        # Create member first
        member = self.create_test_member(
            first_name="SEPA", last_name="Performance", email="sepa.performance@bench.test"
        )
    ...
    def test_chapter_membership_assignment_performance_baseline(self):
        ...
        # Create member first
        member = self.create_test_member(
            first_name="Chapter", last_name="Performance", email="chapter.performance@bench.test"
        )
        ...
        memberships = frappe.get_all("Chapter Member", filters={"member": member.name, "enabled": 1})
    ...
    def test_bulk_member_query_performance_baseline(self):
        ...
        for i in range(5):  # Small batch for fast tests
            member = self.create_test_member(
                first_name=f"Bulk{i}", last_name="Performance", birth_date="1990-01-01"
            )
    ...
    def test_comprehensive_workflow_performance_baseline(self):
        ...
        with self.assertQueryCount(1500):  # Realistic baseline for complete workflow
            ...
            chapter_memberships = frappe.get_all(
                "Chapter Member", filters={"member": member.name, "enabled": 1}
            )
            active_mandates = frappe.get_all(
                "SEPA Mandate", filters={"member": member.name, "status": "Active"}
            )
    ...
    def test_performance_monitoring_and_reporting(self):
        ...
        member = self.create_test_member(
            first_name="Monitor", last_name="Performance", email="monitor.performance@bench.test"
        )
        ...
        self.assertLess(
            duration, 5.0, f"Member creation should complete within 5 seconds, took {duration:.2f}s"
        )
