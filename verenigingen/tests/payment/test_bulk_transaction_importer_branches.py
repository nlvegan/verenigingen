...
class TestCreateBankTransactionResolutionBranches(_BulkImporterSweepBase):
    ...
    def _seed_bank_on_resolved_default(self):
        ...
        Bank Account, and return that account's currency.

        Mirrors the importer's own resolution (user-default Company, else Global
        Defaults default_company) and its bank-account pick order (is_default
        first, then any), so the account seeded/read here is the one the importer
        will use. Reuses the base get-or-create helper, so it is idempotent and
        rolled back with the test transaction. The currency is returned because
        the importer stamps the Bank Transaction with the Mollie payment's
        currency, and ERPNext rejects a BT whose currency differs from its Bank
        Account's — on a CI runner the default company/account is INR, not EUR."""
        ...
        bank_account = self._persist_company_bank_account(resolved)
        gl_account = frappe.db.get_value("Bank Account", bank_account, "account")
        currency = (gl_account and frappe.db.get_value("Account", gl_account, "account_currency")) or (
            frappe.db.get_value("Company", resolved, "default_currency")
        )
        return currency
    ...
    def test_payment_resolves_default_company_and_bank(self):
        ...
        # resolve, and drive the payment currency from that account so the BT the
        # importer builds does not trip ERPNext's currency-mismatch guard on a
        # CI runner whose default company is INR (not EUR).
        currency = self._seed_bank_on_resolved_default()
        self.imp.import_id = f"res_{self.uid}"
        bt = self.imp._create_bank_transaction_from_payment(
            _payment(f"tr_resolve_{self.uid}", amount={"value": "12.00", "currency": currency}),
        ...
        bt = self.imp._create_bank_transaction_from_settlement(
            {
                ...
                "reference": "res-ref",
                "amount": {"value": "300.00", "currency": currency},
            },
            datetime(2024, 3, 15, tzinfo=timezone.utc),
            company=None,
            bank_account=None,
