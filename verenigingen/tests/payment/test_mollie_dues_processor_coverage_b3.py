...
class TestProcessDuesDeprecatedMode(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        # find_member_for_payment's module-global matcher singleton now
        # self-invalidates on a (site, count, max(modified)) fingerprint over
        # Members carrying a mollie_customer_id (#255), so it picks up this
        # member without a manual reset: this member enters that set here,
        # which changes the count component even though the db_set above uses
