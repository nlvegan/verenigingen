...
class TestDuesScheduleHealthManager(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        # Use a low minimum so test schedules with small dues_rate (e.g. €25/€50)
        # don't trip "Dues rate cannot be less than minimum amount".
    ...
    def test_priority_hierarchy_logic(self):
        ...
        member = self.create_test_member(first_name="Priority", last_name="Test", birth_date="1990-01-01")
    ...
    def test_priority_hierarchy_fallback(self):
        ...
        member = self.create_test_member(first_name="Fallback", last_name="Test", birth_date="1990-01-01")
    ...
    def test_transaction_safety_success(self):
        ...
        member = self.create_test_member(
            first_name="Transaction", last_name="Success", birth_date="1990-01-01"
        )
    ...
    def test_transaction_safety_rollback(self):
        ...
        member = self.create_test_member(
            first_name="Transaction", last_name="Failure", birth_date="1990-01-01"
        )
    ...
    def test_custom_rate_preservation(self):
        ...
        member = self.create_test_member(first_name="Custom", last_name="Rate", birth_date="1990-01-01")
        ...
        existing_schedules = frappe.get_all(
            "Membership Dues Schedule", filters={"member": member.name, "status": "Active", "is_template": 0}
        )
        for existing in existing_schedules:
            frappe.db.set_value("Membership Dues Schedule", existing.name, "status", "Cancelled")
    ...
    def test_batch_processing_logic(self):
        ...
        for i in range(5):
            member = self.create_test_member(
                first_name=f"Batch{i}", last_name="Test", birth_date="1990-01-01"
            )
            members.append(member.name)

        # Test batch processing with small batch size
        result = comprehensive_dues_schedule_health_check(
            member_filter=members, fix_issues=False, batch_size=2, continue_on_error=True
        )
    ...
    def test_error_pattern_recognition(self):
        ...
        member = self.create_test_member(first_name="Pattern", last_name="Test", birth_date="1990-01-01")
        ...
        # Test critical manual-review patterns (must match the service's
        # manual_review_patterns list: customer record / account / currency /
        # company / permission denied / access forbidden).
    ...
    def test_field_synchronization_logic(self):
        ...
        member = self.create_test_member(first_name="Sync", last_name="Test", birth_date="1990-01-01")
    ...
    def test_membership_reconstruction(self):
        ...
        # Verify membership was created. reconstruct_missing_membership inserts
        # the record (Draft) but does not submit it, so set_status() keeps it in
        # "Draft" until a reviewer submits it.
    ...
    def test_large_dataset_performance(self):
        ...
        for i in range(10):  # Smaller number for unit test
            member = self.create_test_member(first_name=f"Perf{i}", last_name="Test", birth_date="1990-01-01")
        ...
        result = comprehensive_dues_schedule_health_check(
            member_filter=member_names, fix_issues=False, batch_size=5
        )
    ...
    def test_edge_case_corrupted_data(self):
        ...
        member = self.create_test_member(first_name="Corrupted", last_name="Test", birth_date="1990-01-01")
    ...
    def test_sync_all_member_fields_batch_processing(self):
        ...
        # Create test members
        for i in range(3):
            self.create_test_member(first_name=f"SyncAll{i}", last_name="Test", birth_date="1990-01-01")
...
class TestDuesScheduleHealthManagerIntegration(EnhancedTestCase):
    ...
    def setUp(self):
        super().setUp()
        # Tests reference the "Monthly Membership" Membership Type by literal name.
        # Low minimum keeps the auto-created template rate (€15) above the minimum.
        ensure_membership_type_exists("Monthly Membership", amount=5.0)
    ...
    def test_end_to_end_health_check(self):
        ...
        result = comprehensive_dues_schedule_health_check(
            member_filter=[member.name], fix_issues=True, batch_size=1
        )
    ...
    def test_error_recovery_integration(self):
        ...
        member = self.create_test_member(
            first_name="ErrorRecovery", last_name="Test", birth_date="1990-01-01"
        )
