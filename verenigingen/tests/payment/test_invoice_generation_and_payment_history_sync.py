...
class TestInvoiceGenerationAndPaymentHistorySync(VereningingenTestCase):
    ...
    def setUp(self):
        ...
        self.member = self.create_test_member(
            first_name="Test", last_name="PaymentHistory", email="test.paymenthistory@example.com"
        )
        ...
        # explicit schedule below is the only active one for this member.
    ...
    def test_payment_status_field_allows_draft(self):
        ...
        for option in expected_options:
            self.assertIn(option, options, f"Payment Status field is missing required option: {option}")
    ...
    def test_invoice_due_date_calculation(self):
        ...
        self.assertGreaterEqual(due_date, posting_date, "Due date must be on or after posting date")

        # For invoices without payment terms, due date should be a reasonable
        # span after posting (the default credit days depend on company/customer
        # configuration, so assert a sensible window rather than exactly 30 days).
        if not invoice.payment_terms_template:
            self.assertLessEqual(
                due_date,
                add_days(posting_date, 60),
                "Due date should be within 60 days of posting when no payment terms specified",
            )
    ...
    def test_payment_history_sync_with_draft_invoice(self):
        ...
        invoice.company = frappe.db.get_single_value("Verenigingen Settings", "company")
        ...
        self.assertGreater(
            final_count, initial_count, "Payment history should be updated when invoice is submitted"
        )

        # Verify the specific invoice appears in payment history
        invoice_in_history = any(
            p.invoice == invoice.name for p in self.member.payment_history if hasattr(p, "invoice")
        )
        self.assertTrue(invoice_in_history, "Submitted invoice should appear in member's payment history")
    ...
    def test_end_to_end_automatic_invoice_generation(self):
        ...
        """
        # Generate invoice from dues schedule (returns the Sales Invoice doc)
        generated = self.dues_schedule.generate_invoice()
        self.assertIsNotNone(generated, "Invoice should be generated from dues schedule")
        invoice_name = generated.name if hasattr(generated, "name") else generated
        ...
        self.assertEqual(invoice.customer, self.member.customer, "Invoice should be for the correct customer")
        self.assertEqual(invoice.posting_date, getdate(today()), "Invoice should have today's posting date")
        self.assertGreaterEqual(
            getdate(invoice.due_date), getdate(today()), "Invoice due date should not be in the past"
        )
        ...
        if invoice.docstatus == 1:
            ...
            self.assertTrue(invoice_found, "Auto-submitted invoice should appear in member payment history")
    ...
    def test_payment_history_manual_refresh(self):
        ...
        invoice.append(
            "items",
            {"item_code": "Membership Dues - Daily", "qty": 1, "rate": 2.0, "description": "Test dues"},
        )
        ...
        invoice_in_history = any(
            hasattr(p, "invoice") and p.invoice == invoice.name for p in self.member.payment_history
        )
        self.assertTrue(invoice_in_history, "Invoice should appear in payment history after manual refresh")
    ...
    def test_dues_schedule_invoice_generation_due_date_logic(self):
        ...
        self.assertNotEqual(
            getdate(invoice.due_date), getdate(past_date), "Due date should NOT be set to next_invoice_date"
        )

        # Verify due date is in the future
        self.assertGreaterEqual(
            getdate(invoice.due_date), getdate(today()), "Due date should be in the future, not past"
        )
...
class TestPaymentHistoryFieldValidation(VereningingenTestCase):
    ...
    def test_payment_status_field_validation_comprehensive(self):
        ...
        member = self.create_test_member(
            first_name="Status", last_name="Test", email="status.test@example.com"
        )
        ...
        self.assertTrue(validation_success, f"All payment statuses should be valid. Error: {error_message}")
