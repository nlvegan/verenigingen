...
class TestInvoiceEligibilityValidation(EnhancedTestCase):
    ...
    def setUp(self):
        """Set up test data"""
        super().setUp()
        # Tests reference the "Test Membership" type by literal name; ensure it
        # exists and is active with a low minimum so the auto-created template
        # rate (€15) stays above the minimum.
        from verenigingen.tests.fixtures.test_data_factory import ensure_membership_type_exists

        ensure_membership_type_exists("Test Membership", amount=5.0)
    ...
    def test_member_with_sepa_direct_debit_no_mandate_gets_invoice(self):
        ...
        member = self.create_test_member(
            ...
            bank_account_name="SEPANoMandate TestMember",
        )

        # Create active membership
        membership = self.create_test_membership(
            member=member.name, membership_type="Test Membership", status="Active"
        )
        ...
        dues_schedule.currency = "EUR"
        dues_schedule.billing_frequency = "Monthly"
        dues_schedule.dues_rate = 15.0
        dues_schedule.next_invoice_date = today()
        dues_schedule.status = "Active"
        self._cancel_auto_schedules(dues_schedule.member)
        dues_schedule.save()
        self.track_test_record("Membership Dues Schedule", dues_schedule.name)
        ...
        self.assertTrue(
            is_eligible, "Member with SEPA Direct Debit but no mandate should be eligible for invoicing"
        )
        ...
        self.track_test_record("Sales Invoice", invoice_name)
    ...
    def test_member_without_active_membership_blocked_from_invoicing(self):
        ...
        # Create a membership, then create the dues schedule while it is active,
        # and only afterwards force the membership to Expired. The factory always
        # submits the membership (so it is briefly Active), which is required for
        # the schedule insert; we then expire it to model "no active membership".
        membership = self.create_test_membership(
            member=member.name,
            membership_type="Test Membership",
            status="Active",
        ...
        # Create dues schedule (member still has an active membership here)
        dues_schedule = frappe.new_doc("Membership Dues Schedule")
        dues_schedule.schedule_name = f"Test Schedule {member.name}"
        dues_schedule.member = member.name
        dues_schedule.membership_type = "Test Membership"
        ...
        # Now expire the membership so the member has no ACTIVE membership.
    ...
    def test_member_with_active_sepa_mandate_works_normally(self):
        ...
        member = self.create_test_member(
            ...
            payment_method="SEPA Direct Debit",
            iban="NL13TEST0123456789",
            bank_account_name="ValidSEPA TestMember",
        )
        ...
        mandate = self.create_test_sepa_mandate(
            member_name=member.name, iban="NL13TEST0123456789", status="Active", is_active=1
        )
    ...
    def test_bulk_invoice_generation_excludes_ineligible_members(self):
        ...
        broken_sepa_member = self.create_test_member(
            ...
            payment_method="SEPA Direct Debit",
            iban="NL13TEST0123456789",
            bank_account_name="BrokenSEPA BulkTest",
        )
        ...
        # Assertions
        self.assertTrue(
            valid_schedule.validate_member_eligibility_for_invoice(), "Valid SEPA member should be eligible"
        )
        self.assertTrue(
            broken_schedule.validate_member_eligibility_for_invoice(),
            "Broken SEPA member should STILL be eligible (new behavior)",
        )
        self.assertFalse(
            terminated_schedule.validate_member_eligibility_for_invoice(),
        ...
        broken_invoice = (
            broken_invoice_doc.name if hasattr(broken_invoice_doc, "name") else broken_invoice_doc
        )
        self.track_test_record("Sales Invoice", valid_invoice)
        self.track_test_record("Sales Invoice", broken_invoice)
