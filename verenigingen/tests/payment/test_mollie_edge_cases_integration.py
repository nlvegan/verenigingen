...
from verenigingen.tests.payment.mollie_api_data_factory import (
...
def _setup_mollie_backend_api():
    ...
    to construct unless ``enable_backend_api`` is set and an
    ``organization_access_token`` is present. A freshly reset test site has
    neither, so the tests below turn both on with a dummy token. The Mollie
    SDK / HTTP layer is mocked in every test, so the dummy token is never sent
    anywhere - it only satisfies the in-app "is configured" gate. The config
    service caches settings for 5 minutes, so its cache is cleared too.
    """
...
class TestMollieEdgeCasesIntegration(EnhancedTestCase):
    ...
    def setUp(self):
        ...
        # Set up security manager. MollieSecurityManager.__init__ reads the
        # stored encryption_key via settings.get_password and urlsafe-b64decodes
        # it back into the Fernet key, so the mock must return that key wrapped
        # in one extra b64 layer (mirroring how the manager stores it). A bare
        # Mock would blow up the b64decode. Mock justified: Mollie Settings is
        # the external config boundary; the crypto below runs for real against
        # this fixed test key.
        import base64
        ...
        self.mock_settings.get_password = Mock(
            return_value=base64.urlsafe_b64encode(fernet_key).decode("utf-8")
        )
    ...
    def test_concurrent_api_calls_thread_safety(self):
        ...
        # frappe.local is thread-isolated (werkzeug Local), so a worker thread
        # starts with no site context - get_dashboard_summary would raise
        # AttributeError('conf') the moment it touches frappe.logger()/db.
        # Each thread must init + connect to the current site itself, then tear
        # the connection down. The dashboard's Mollie clients are mocked, so the
        # only real work in the thread is the in-memory aggregation under test.
    ...
    def test_security_manager_dashboard_integration(self):
        ...
        webhook_payload = self.factory.generate_webhook_payload(
            resource_type="settlement", include_signature_data=True
        )
        ...
        with patch.object(self.security_manager, "_create_audit_log"):
            ...
            result = self.security_manager.validate_webhook_signature(
                webhook_payload["payload"], expected_signature
            )
    ...
    def test_extreme_settlement_amounts(self):
        ...
        extreme_settlements = [
            # Very small amount
            {**self.factory.generate_settlement_data(), "amount": {"value": "0.01", "currency": "EUR"}},
            # Very large amount
            {**self.factory.generate_settlement_data(), "amount": {"value": "999999.99", "currency": "EUR"}},
            # Zero amount (edge case)
            {**self.factory.generate_settlement_data(), "amount": {"value": "0.00", "currency": "EUR"}},
            # Negative amount (chargeback scenario)
            {**self.factory.generate_settlement_data(), "amount": {"value": "-100.00", "currency": "EUR"}},
        ]
    ...
    def test_malformed_settlement_data_resilience(self):
        ...
        malformed_settlements = [
            ...
            {"unexpected": "structure", "not_a_settlement": True},
        ]
...
class TestMollieApiParameterValidation(EnhancedTestCase):
    ...
    def test_api_parameter_sanitization(self):
        ...
        # get() must return a non-empty list of valid settlement dicts -
        # list_settlements parses the response into Settlement objects, and both
        # a bare Mock and an empty list would fail that parse ("Empty response").
        client.get = Mock(return_value=[self.factory.generate_settlement_data(status="paidout")])
        ...
        for params in test_params:
            ...
            # use_cache=False forces every call down to self.get; otherwise the
            # second iteration would hit the cache and never reach the mock,
            # defeating the parameter-sanitization assertions below.
            client.list_settlements(use_cache=False, **params)
    ...
    def test_date_range_validation(self):
        ...
        test_cases = [
            ...
            {"from_date": datetime(2025, 7, 1), "until_date": datetime(2025, 8, 31), "expected_count": 2},
            # Narrow range - should include only one
            {"from_date": datetime(2025, 7, 20), "until_date": datetime(2025, 8, 31), "expected_count": 1},
            # No overlap - should include none
            {"from_date": datetime(2025, 9, 1), "until_date": datetime(2025, 9, 30), "expected_count": 0},
        ]

        for test_case in test_cases:
            result = client.list_settlements(
                from_date=test_case["from_date"], until_date=test_case["until_date"]
            )
