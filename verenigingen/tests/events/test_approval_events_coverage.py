...
class TestApprovalEventsCoverage(EnhancedTestCase):
    ...
    def test_emit_event_enqueues_when_not_sync(self):
        ...
        job_name/delay are no longer passed to frappe.enqueue: dispatch timing
        is handled by enqueue_after_commit=True + deduplicate=True, and the
        job_id is namespaced per-subscriber (see
        test_emit_event_enqueues_one_job_per_subscriber for the regression
        this guards against).
        """
        calls = []
        ...
        self.assertTrue(enqueued["enqueue_after_commit"])
