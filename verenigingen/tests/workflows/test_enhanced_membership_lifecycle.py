...
from verenigingen.tests.utils.skip_reasons import (
    ENHANCED_MEMBERSHIP_API_GONE as _ENHANCED_MEMBERSHIP_API_GONE,
    LIFECYCLE_SCHEMA_GONE as _LIFECYCLE_SCHEMA_GONE,
)
...
class TestEnhancedMembershipLifecycle(EnhancedTestCase):
    ...
    def test_calculator_based_membership_complete_workflow(self):
        ...
        member = frappe.get_doc("Member", application_id)
        self.track_doc("Member", member.name)

        # 2. Validate calculator configuration
        self.assertEqual(member.contribution_mode, "Calculator")
        self.assertEqual(member.base_multiplier, 1.5)
        expected_amount = self.calculator_membership_type.suggested_contribution * 1.5
        self.assertEqual(member.contribution_amount, expected_amount)

        # 3. Complete workflow - member already exists, just need to activate
        member.status = "Active"  # Approve the member
        member.save()
