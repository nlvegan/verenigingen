...
# TestTimingCheckWithCoverageGaps was removed here along with
# EligibilityChecker.check_schedule_timing(). Its three tests pinned the
# next_invoice_date / invoice_days_before guard, which no longer exists: eligibility
# no longer decides WHEN to generate, only WHETHER the member is billable. The cutoff
# comparison in should_generate_invoice_for_cutoff() is now the single control, and it
# is covered by TestCoverageBasedEligibility above and by
# TestCutoffIsCappedAtOnePeriodAhead in test_eligibility_checker.py.
#
# The property those tests protected - a coverage GAP must never block generation - is
# preserved structurally rather than by a guard: a lapsed coverage_end is by definition
# earlier than the cutoff, so the cutoff comparison returns True on its own.
