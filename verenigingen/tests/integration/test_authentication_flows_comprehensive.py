...
class TestAuthenticationFlowsComprehensive(EnhancedTestCase):
    ...
    def test_portal_page_access_controls(self):
        ...
        with self.as_user(self.test_users['member_full'].email):
            # Simulate portal page permission check
            from verenigingen.templates.pages.bank_details import has_website_permission
            
            # Member should have access to bank details page
    ...
    def test_portal_context_generation_security(self):
        ...
        with self.as_user(self.test_users['member_full'].email):
            # Import and test portal context generation
            ...
            self.assertIn('csrf_token', result_context)
    ...
    def test_authentication_error_handling(self):
        ...
        with patch('frappe.db.get_value', side_effect=Exception("Database error")):
            result = get_member_name_for_user(self.test_users['member_full'].email)
            self.assertIsNone(result)  # Should return None, not raise exception
