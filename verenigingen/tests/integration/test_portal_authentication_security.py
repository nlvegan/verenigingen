...
class TestPortalAuthenticationSecurity(EnhancedTestCase):
    ...
    # ===== BANK DETAILS PORTAL AUTHENTICATION TESTS =====

    def test_bank_details_portal_access_control(self):
        """Test bank details portal access control and authentication"""
        
        # Test authorized member access
        with self.as_user(self.portal_users['portal_financial'].email):
            from verenigingen.templates.pages.bank_details import has_website_permission
            
            # Should have access to bank details
    ...
    def test_bank_details_portal_context_security(self):
        """Test bank details portal context generation security"""
        
        with self.as_user(self.portal_users['portal_financial'].email):
            from verenigingen.templates.pages.bank_details import get_context
            
            context = {}
            result = get_context(context)
            
            # Verify security elements are present
            self.assertIn('csrf_token', result, "CSRF token should be included")
            ...
            # Verify financial data is properly loaded
            if hasattr(member, 'iban') and member.iban:
                self.assertIn('current_details', result, "Financial details should be loaded")
                details = result['current_details']
                self.assertIn('iban', details, "IBAN should be in context")

    def test_bank_details_portal_session_validation(self):
        """Test bank details portal session validation"""
    ...
    def test_bank_details_api_authentication(self):
        """Test bank details API endpoint authentication"""
        
        with self.as_user(self.portal_users['portal_financial'].email):
            from verenigingen.templates.pages.bank_details import test_bank_details_api
            
            # API should work with authenticated user
            result = test_bank_details_api()
            self.assertTrue(result['success'], "API should succeed with authenticated user")
            self.assertTrue(result['found_member'], "Should find member for authenticated user")
            self.assertEqual(result['user'], frappe.session.user, "Should return correct user")
            
        # Test API with guest user (should fail)
        with self.as_user("Guest"):
            from verenigingen.templates.pages.bank_details import test_bank_details_api
            
            result = test_bank_details_api()
            self.assertEqual(result['user'], "Guest", "Should identify Guest user")
            self.assertFalse(result['found_member'], "Should not find member for Guest")
    ...
    def test_payment_dashboard_portal_authentication(self):
        ...
        with self.as_user(self.portal_users['portal_financial'].email):
            # Simulate payment dashboard access
            from verenigingen.templates.pages.payment_dashboard import has_website_permission
    ...
    def test_payment_dashboard_context_security(self):
        ...
        with self.as_user(self.portal_users['portal_financial'].email):
            ...
            # Verify member-specific context
            self.assertIn('member', result)
            member = result['member']
    ...
    def test_portal_csrf_protection_integration(self):
        ...
        with self.as_user(self.portal_users['portal_full'].email):
            # Verify CSRF token is generated for portal pages
            from verenigingen.templates.pages.bank_details import get_context
            
            context = {}
            result = get_context(context)
            
            self.assertIn('csrf_token', result, "CSRF token should be present")
            self.assertIsNotNone(result['csrf_token'], "CSRF token should not be None")
    ...
    def test_portal_database_error_handling(self):
        """Test portal handling of database errors"""
        
        with self.as_user(self.portal_users['portal_full'].email):
            # Mock database error during member lookup
            with patch('frappe.db.get_value', side_effect=Exception("Database error")):
                from verenigingen.utils.member_utils import get_member_name_for_user
                
                # Should handle error gracefully
                result = get_member_name_for_user(frappe.session.user)
