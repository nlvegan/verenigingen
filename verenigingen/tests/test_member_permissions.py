...
class TestMemberPermissions(EnhancedTestCase):
    ...
    def test_staff_sees_all_members(self):
        ...
        members = frappe.get_all("Member", filters={"name": ["in", [
            self.member1.name, self.member2.name, self.member3.name
        ]]})
    ...
    def test_user_permission_does_not_filter_members(self):
        ...
        self.assertEqual(condition, "",
            "Staff permission query should not be affected by Employee User Permission")

        # Verify can still see members without employee link
        members_without_employee = frappe.get_all("Member",
            filters={"name": ["in", [self.member2.name, self.member3.name]]})

        self.assertEqual(len(members_without_employee), 2,
    ...
    def test_no_duplicate_permission_query_hooks(self):
        ...
        self.assertEqual(len(member_hooks), 1,
            "Should have exactly one permission query hook for Member")
...
class TestMemberPermissionQueryPerformance(EnhancedTestCase):
    ...
    def test_permission_query_does_not_cause_n_plus_1(self):
        ...
        with self.assertQueryCount(50):  # Adjust based on actual query count
            ...
            # Fetch all members
            all_members = frappe.get_all("Member",
                filters={"name": ["in", [m.name for m in members]]})
