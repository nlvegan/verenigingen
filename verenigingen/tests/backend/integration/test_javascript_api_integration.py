...
class JavaScriptAPIIntegrationTestCase(VereningingenTestCase):
            ...
            if api_file.exists():
                ...
                if f"def {function_name}" in content:
                    ...
                    if function_match:
                        ...
                        int_usage = re.findall(
                            r"int\s*\(\s*([a-zA-Z_][a-zA-Z0-9_.*\[\]]*)\s*\)", func_content
                        )
                        if int_usage:
    ...
    def test_api_function_whitelist(self):
        ...
        for filename, function_name in critical_api_calls:
            ...
            if api_file.exists():
                ...
                # Check if function exists
                if f"def {function_name}" in content:
                    # Check if it's whitelisted. @frappe.whitelist() is the
                    # outermost decorator, but security/monitoring decorators
                    # sit between it and the def, so allow intervening decorator
                    # lines (each starting with @) before the function.
                    whitelist_pattern = (
    ...
    def test_member_doctype_boolean_conversions(self):
        ...
        if member_file.exists():
            ...
            critical_conversions = ["used_for_memberships", "used_for_donations", "send_welcome_email"]
            ...
            for param in critical_conversions:
                ...
                if param in content and f"cbool({param})" not in content:
                    ...
                    if f"{param} =" in content or f".{param} =" in content:
                        self.fail(f"Member doctype should use cbool({param}) for boolean conversion")
