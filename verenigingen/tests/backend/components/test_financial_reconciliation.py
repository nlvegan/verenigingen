...
class TestFinancialReconciliationComprehensive(VereningingenTestCase):
    ...
    def create_test_member_with_financial_setup(self):
        ...
        # Create membership type + active membership so the dues schedule passes
        # its validate_member_membership check (member must have active membership).
        # Leave minimum_amount at the helper default so the auto-created dues
        # template (rate 15.0) does not violate the minimum-amount constraint.
        ...
        # CoreTestDataFactory.create_test_membership inserts but does not submit;
        # the dues schedule's validate_member_membership requires docstatus=1.
        if membership.docstatus == 0:
            membership.submit()

        # Submitting the membership auto-creates an active dues schedule
        # (Membership.on_submit). Reuse it instead of creating a duplicate, which
        # would trip the "already has an active dues schedule" guard.
        ...
        # The membership submit and dues schedule save run hooks that touch the
        # Member row, so the in-memory member is now stale. Reload before the
        # tests mutate and save it (otherwise TimestampMismatchError).
        member.reload()

        return member

    def setup_test_accounts(self):
        """Resolve real accounts on the EUR test company.

        The previous hardcoded "* - TC" names assumed a company that is never
        created on a fresh test site; resolve the actual chart-of-accounts of the
        ...
        def acct(account_type):
            return frappe.db.get_value(
                "Account",
                {"account_type": account_type, "company": self.test_company, "is_group": 0},
                "name",
            )
        ...
        self.accounts = {
            ...
            "cash_account": acct("Cash") or acct("Bank"),
            # These two are referenced only by the fully-mocked helper return
            # values, not by any real DB write, so a label is sufficient.
    ...
    def test_termination_with_outstanding_invoices_reconciliation(self):
        ...
        reconciliation = self.process_termination_reconciliation(
            member.name, outstanding_invoices, reconciliation_method="write_off_with_approval"
        )
        ...
        bad_debt_entry = next(
            (je for je in journal_entries if je.get("account") == self.accounts["bad_debt_expense"]), None
        )
    ...
    def test_retroactive_termination_financial_adjustment(self):
        ...
        post_termination_payment = self.create_test_payment(
            member, amount=25.0, payment_date=add_days(today(), -18), invoice=post_termination_invoice.name
        )
        ...
        adjustment = self.process_retroactive_termination_adjustment(member.name, retroactive_date)
    ...
    def test_suspension_grace_period_financial_handling(self):
        ...
        grace_payment_processing = self.process_payment_during_grace_period(
            member.name, grace_period_invoice.name
        )
    ...
    def test_member_reactivation_financial_restoration(self):
        ...
        restoration = self.process_reactivation_financial_restoration(member.name, suspension_state)
    ...
    def test_reactivation_with_outstanding_balance_reconciliation(self):
        ...
        for i in range(2):
            invoice = self.create_test_invoice(
                member, amount=25.0, posting_date=add_days(today(), -45 + i * 15), payment_status="Unpaid"
            )
        ...
        reactivation_payment = self.process_reactivation_payment(
            member.name, amount=50.0, payment_method="Bank Transfer", reference="Reactivation payment"
        )
    ...
    def test_sepa_mandate_status_change_financial_impact(self):
        ...
        # Get SEPA mandate
        mandate_name = frappe.get_value("SEPA Mandate", {"member": member.name}, "name")
        ...
        cancellation_impact = self.process_mandate_cancellation_impact(mandate.name, scheduled_transactions)
    ...
    def test_payment_method_change_financial_reconciliation(self):
        ...
        payment_method_change = self.process_payment_method_change(
            member.name, initial_payment_method, new_payment_method, change_reason="SEPA mandate issues"
        )
        ...
        # Note: Membership Dues Schedule has no `payment_method` field (payment
        # method lives on the Member / SEPA Mandate, not the schedule), so there
        # is nothing to assert on the schedule here.
    ...
    def test_status_change_revenue_recognition_impact(self):
        ...
        revenue_adjustment = self.process_revenue_recognition_adjustment(
            member.name, advance_payment, termination_date
        )
        ...
        deferred_revenue_entry = next(
            (je for je in journal_entries if "Deferred Revenue" in je.get("account", "")), None
        )
    ...
    def test_partial_period_billing_reconciliation(self):
        ...
        partial_reconciliation = self.process_partial_period_reconciliation(
            member.name, full_period_invoice.name, period_start, termination_date
        )
    ...
    def analyze_termination_financial_impact(self, member_name, termination_date):
        ...
        return {
            "outstanding_invoices": {"count": 2, "total_amount": 50.0, "oldest_date": add_days(today(), -60)},
            "future_revenue_loss": {"monthly_amount": 25.0, "annual_projection": 300.0},
            "refund_obligations": {"total_amount": 0.0, "advance_payments": []},
            "sepa_mandate_impact": {"requires_cancellation": True, "pending_transactions": 1},
        }
    ...
    def create_test_invoice(
        ...
        # Honour the back-dated posting_date; without this flag ERPNext resets
        # posting_date to today while leaving the (past) due_date, which trips
        ...
        # No tax template: the reconciliation assertions expect net == rate.
    ...
    def create_test_payment(self, member, amount, payment_date, invoice=None):
        ...
        payment.company = self.test_company
    ...
    def process_termination_reconciliation(self, member_name, outstanding_invoices, reconciliation_method):
        ...
        # Use the pre-tax net_total: the test company's default sales-tax template
        # adds incidental tax that is irrelevant to the membership-dues amount the
        # reconciliation is written off against.
        total_amount = sum(flt(inv.net_total) for inv in outstanding_invoices)
        ...
        return {
            ...
            "journal_entries": [
                {"account": self.accounts["bad_debt_expense"], "debit_amount": total_amount},
                {"account": self.accounts["accounts_receivable"], "credit_amount": total_amount},
            ],
        }
    ...
    def process_retroactive_termination_adjustment(self, member_name, retroactive_date):
        ...
        return {
            ...
            "refund_amount": 25.0,
            "journal_entries": [
                {"type": "refund", "account": self.accounts["refund_liability"], "amount": 25.0}
            ],
        }
    ...
    def process_sepa_batch_with_status_validation(self, batch_id):
        ...
        return {
            ...
            "excluded_payments": [
                {"member": self.test_member.name, "amount": 75.0, "exclusion_reason": "member_suspended"}
            ],
            "financial_impact": {"processed_amount": 50.0, "excluded_amount": 75.0},
        }

    def process_payment_during_grace_period(self, member_name, invoice_name):
        """Process payment during grace period"""
        return {"allowed": True, "special_conditions": ["grace_period_conditions"], "additional_fees": 0.0}

    def process_payment_after_grace_expiry(self, member_name, current_date):
        """Process payment after grace period expiry"""
        return {"allowed": False, "restriction_reason": "grace_period_expired"}
    ...
    def process_reactivation_financial_restoration(self, member_name, suspension_state):
        ...
        return {
            "success": True,
            "actions": ["dues_schedule_reactivated", "payment_method_restored", "billing_cycle_resumed"],
        }
    ...
    def process_reactivation_payment(self, member_name, amount, payment_method, reference):
        """Process payment for reactivation"""
        return {"success": True, "amount_applied": amount, "payment_reference": reference}

    def get_member_financial_state(self, member_name):
        """Get current financial state of member"""
        return {"outstanding_balance": 0.0, "status": "Current", "payment_method": "SEPA Direct Debit"}
    ...
    def process_revenue_recognition_adjustment(self, member_name, advance_payment, termination_date):
        ...
        return {
            ...
            "journal_entries": [
                {"account": "Deferred Revenue - TC", "debit_amount": remaining},
                {"account": self.accounts["refund_liability"], "credit_amount": remaining},
            ],
        }
