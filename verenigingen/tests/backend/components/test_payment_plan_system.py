...
class TestPaymentPlanSystem(VereningingenTestCase):
    ...
    def test_payment_plan_api_request(self):
        ...
        request_payment_plan is decorated @self_service_api(FINANCIAL) (LOW tier):
        a plain member may request their OWN plan, and the framework enforces that
        the `member=` argument matches the caller. Exercised here as Administrator,
        whose access is unconditional; member-as-self coverage lives in
        tests/workflows/test_member_portal_self_service.py.
        """
