...
class TestAgeValidator(EnhancedTestCase):
    ...
    def test_student_membership_age_validation(self):
        ...
        result = AgeValidator.validate_age(young_birth_date, context="student_membership", throw_on_error=False)
        self.assertFalse(result.is_valid)
    ...
    def test_configurable_settings_integration(self):
        """Test that settings from Verenigingen Settings are used"""
        # This test validates that the _get_configurable_min_age method
        # correctly reads from the Verenigingen Settings DocType
        
        # Test that the method handles missing settings gracefully
        min_age = AgeValidator._get_configurable_min_age("membership", 16)
        self.assertIsInstance(min_age, int)
        self.assertGreaterEqual(min_age, 12)  # Reasonable minimum
        self.assertLessEqual(min_age, 25)     # Reasonable maximum
