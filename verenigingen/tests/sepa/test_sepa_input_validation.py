...
class TestSEPAInputValidation(VereningingenTestCase):
    ...
    def setUp(self):
        ...
        # Create test data
        self.member = self.create_test_member(first_name="TestSEPA", email="sepa@example.com")

        self.ensure_test_membership_type("Monthly Standard")
        self.membership = self.create_test_membership(
            member=self.member.name, membership_type="Monthly Standard"
        )
    ...
    def test_validate_collection_date_weekend(self):
        ...
        # Find next Saturday (weekday 5). frappe.utils.add_days returns a string
        # in v16, so wrap in getdate() before calling date methods.
        test_date = getdate(add_days(today(), 1))
        while test_date.weekday() != 5:  # Saturday
            test_date = getdate(add_days(test_date, 1))
            if (test_date - getdate(today())).days > 30:  # Safety check
    ...
    def test_validate_amount_valid(self):
        ...
        valid_amounts = ["25.00", "100.50", "1.23", 25.00, 100.50, 1.23, Decimal("25.00"), Decimal("100.50")]
