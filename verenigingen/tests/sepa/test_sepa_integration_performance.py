...
class TestSEPAIntegrationPerformance(EnhancedTestCase):
    ...
    def test_complete_sepa_workflow_with_optimizations(self):
        ...
        scenario = self.sepa_factory.create_sepa_test_scenario(
            scenario_name="performance_test", member_count=10
        )
        ...
        critical_errors = [e for e in self.error_handler.error_log if e.severity.value == "critical"]
    ...
    def test_error_handling_during_performance_optimization(self):
        ...
        scenario = self.sepa_factory.create_sepa_test_scenario(scenario_name="error_test", member_count=3)
        ...
        metrics = [
            m for m in self.performance_monitor.metrics_history if m.operation == "error_handling_test"
        ]
        self.assertGreater(len(metrics), 0)

    def test_performance_degradation_handling(self):
        """When the performance optimizer raises, add_invoices_to_batch_optimized
        must fall back to _add_invoices_to_batch_fallback() and actually add the
        invoices -- not just "not crash". The former version of this test wrapped
        the call in a try/except that swallowed any outcome and asserted
        assertTrue(True) in the success branch, so it passed whether or not the
        fallback did anything at all.
        """
        from verenigingen.verenigingen_payments.doctype.direct_debit_batch.sepa_processor import SEPAProcessor

        processor = SEPAProcessor()

        # Create a real scenario (members/mandates/memberships/invoices) so the
        # fallback path has complete, valid data to append to the batch with.
        scenario = self.sepa_factory.create_sepa_test_scenario(
            scenario_name="degradation_test", member_count=2
        )

        mock_invoices = []
        for member, mandate, membership, invoice in zip(
            scenario["members"], scenario["mandates"], scenario["memberships"], scenario["invoices"]
        ):
        ...
        with patch.object(
            ...
            "process_batch_invoices_optimized",
        ...
        self.assertEqual(len(batch.invoices), invoices_before + len(mock_invoices))
        added_invoice_names = {row.invoice for row in batch.invoices}
        self.assertEqual(added_invoice_names, {inv["name"] for inv in mock_invoices})
    ...
    def test_memory_efficiency_large_batch(self):
        ...
        scenario = self.sepa_factory.create_sepa_test_scenario(
            scenario_name="memory_test", member_count=15  # Modest size for test performance
        )
        ...
        memory_metrics = [
            m for m in self.performance_monitor.metrics_history if m.operation == "memory_efficiency_test"
        ]
    ...
    def test_performance_recommendations_generation(self):
        ...
        scenario = self.sepa_factory.create_sepa_test_scenario(
            scenario_name="recommendations_test", member_count=5
        )
        ...
        with patch.object(
            ...
            with monitor_sepa_operation("recommendations_test", batch_size=len(invoice_names)):
                self.performance_optimizer.process_batch_invoices_optimized(invoice_names)
    ...
    def test_api_endpoints_integration(self):
        ...
        from verenigingen.verenigingen_payments.utils.financial_error_handler import (
            get_financial_error_statistics,
        )
    ...
    def test_complete_error_to_recovery_workflow(self):
        ...
        scenario = self.sepa_factory.create_sepa_test_scenario(scenario_name="error_recovery", member_count=2)
        ...
        workflow_metrics = [
            m for m in self.performance_monitor.metrics_history if m.operation == "error_recovery_workflow"
        ]
