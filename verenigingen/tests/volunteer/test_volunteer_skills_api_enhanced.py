...
class TestVolunteerSkillsAPIEnhanced(EnhancedTestCase):
    ...
    def test_business_rules_enforcement(self):
        ...
        # Create a young member. Use a birth_date that keeps the member under 16 relative
        # to the current date (a fixed past date like 2010 eventually ages past 16).
        from frappe.utils import add_years, getdate
        too_young_birth_date = add_years(getdate(), -10)  # 10 years old
        with self.assertRaises(Exception) as cm:
            young_member = self.create_test_member(
                first_name="Young",
                last_name="Verenigingen Volunteer",
                birth_date=str(too_young_birth_date)
            )
