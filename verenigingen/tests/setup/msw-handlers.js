...
const mollieHandlers = [
    ...
    rest.post('https://api.mollie.com/v2/payments', async (req, res, ctx) => {
    ...
    rest.get('https://api.mollie.com/v2/payments/:paymentId', (req, res, ctx) => {
        ...
        return res(
            ctx.json(generateMolliePayment(paymentId, status, '25.00'))
        );
    }),
    ...
    rest.get('https://api.mollie.com/v2/settlements/:settlementId', (req, res, ctx) => {
        ...
        return res(
            ctx.json(generateMollieSettlement(settlementId, 'paidout', '123.45'))
        );
    }),
...
const networkHandlers = [
    ...
    rest.get('https://api.mollie.com/v2/payments/tr_slow_network', (req, res, ctx) => {
        return new Promise(resolve => {
            setTimeout(() => {
                resolve(res(
                    ctx.json(generateMolliePayment('tr_slow_network', 'paid', '25.00'))
                ));
            }, 2000); // 2 second delay
        });
    }),
    ...
    rest.get('https://api.mollie.com/v2/payments/tr_flaky_network', (req, res, ctx) => {
        ...
        if (Math.random() > 0.5) {
            return res(
                ctx.json(generateMolliePayment('tr_flaky_network', 'paid', '25.00'))
            );
        } else {
