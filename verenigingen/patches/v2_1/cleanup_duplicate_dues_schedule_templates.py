    ...
    if frappe.flags.in_migrate:
        ...
        for old_template, new_template in templates_to_update.items():
            membership_types_using_old = frappe.get_all(
                "Membership Type",
                filters={"dues_schedule_template": old_template},
                fields=["name"]
            )
        ...
        for template_name in duplicates_to_remove:
            try:
                ...
                active_schedules = frappe.get_all(
                    "Membership Dues Schedule",
                    filters={
                        "template_reference": template_name,
                        "is_template": 0
                    }
                )
                
                if active_schedules:
                    print(f"Skipping {template_name} - still referenced by {len(active_schedules)} active schedules")
                    continue
