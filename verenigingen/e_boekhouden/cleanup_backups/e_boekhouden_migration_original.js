...
frappe.ui.form.on('E-Boekhouden Migration', {
	refresh(frm) {
		...
		console.log(
			...
			frm.doc.docstatus,
			'Migration status:',
			frm.doc.migration_status
		);

		// Check if we have any migration history
		const has_migrations
      = frm.doc.imported_records > 0 || frm.doc.total_records > 0;
		...
		if (frm.doc.docstatus === 0) {
			...
			if (frm.doc.migration_status !== 'In Progress') {
				...
				frm
					.add_custom_button(__('Test Connection'), () => {
						console.log('Test Connection clicked');
						frappe.call({
							method:
                'verenigingen.e_boekhouden.utils.eboekhouden_api.test_api_connection',
							callback(r) {
				...
				frm
					.add_custom_button(__('Preview Migration'), () => {
						...
						frappe.confirm(
							__(
								'This will preview what data would be migrated without actually importing anything. Continue?'
							),
							() => {
								...
								frappe.call({
									method:
                    'verenigingen.e_boekhouden.doctype.e_boekhouden_migration.e_boekhouden_migration.start_migration_api',
									args: {
									...
									callback(r) {
										...
										if (r.message && r.message.success) {
											...
											frappe.show_alert({
												message: __(
													'✅ Preview migration started successfully!'
												),
												indicator: 'green'
											});
                          ...
                          + (r.message ? r.message.error : 'Unknown error'),
									...
									error(error) {
										...
										frappe.show_alert({
											message:
                        __('❌ API call failed: ') + (error.message || error),
											indicator: 'red'
										});
									}
								});
				...
				frm
					.add_custom_button(__('Start Migration'), () => {
						...
						frappe.confirm(
							__(
								'Are you sure you want to start the migration? This will import data from e-Boekhouden into ERPNext.<br><br><strong>This action cannot be undone!</strong>'
							),
							() => {
				...
				frm
					.add_custom_button(__('Full Migration'), () => {
						...
						frappe.confirm(
							__(
								'<h4>Full E-Boekhouden Migration</h4>'
                          ...
                          + (r.message ? r.message.error : 'Unknown error'),
									...
									error(error) {
										...
										frappe.msgprint({
											...
											message:
                        __('Full migration error: ') + (error.message || error),
											indicator: 'red'
										});
									}
								});
				...
				frm.add_custom_button(
                    ...
                    'verenigingen.utils.fix_receivable_payable_entries.analyze_and_fix_entries',
									callback(r) {
										...
										if (r.message && r.message.success) {
											if (
												r.message.action === 'preview'
                        && r.message.accounts_to_fix
											) {
												// Show preview dialog
                                ...
                                'verenigingen.utils.fix_receivable_payable_entries.apply_account_type_fixes',
                                  ...
                                  && fix_result.message.success
									...
									error(err) {
										console.error(
											'Error calling analyze_and_fix_entries:',
											err
										);
										frappe.msgprint({
											title: __('Error'),
											message: __(
												'Failed to call the analysis method. Check console for details.'
											),
											indicator: 'red'
										});
									}
			...
			frm
				.add_custom_button(__('Reset to Draft'), () => {
					frappe.confirm(
						__(
							'This will reset the migration status to Draft so you can try again. Continue?'
						),
						() => {
							frappe.call({
		...
		if (frm.doc.migration_status === 'Draft' && !has_migrations) {
			...
			frm.set_intro(
				__(
					'Migration is currently running. Progress will be updated automatically.'
				)
			);
		} else if (frm.doc.migration_status === 'Completed' || has_migrations) {
			frm.set_intro(
				__(
					'Migration completed successfully. You can now use the Post-Migration tools to:<br>• Map account types for proper ERPNext functionality<br>• Fix any Receivable/Payable entries missing party information<br>• View detailed migration history<br><br>You can also run additional migrations if needed.'
				),
				'green'
			);
		} else if (frm.doc.migration_status === 'Failed') {
			frm.set_intro(
				__(
					'Migration failed. Check the error log for details. You can reset to Draft to try again.'
				),
				'red'
			);
		}
...
function show_account_mapping_dialog(analysis_data) {
	...
	mapping_fields.push({
		fieldtype: 'HTML',
		options:
      '<div class="mapping-container" style="height: 350px; overflow-y: auto; padding: 10px; margin-bottom: 20px; border: 1px solid #d1d8dd; border-radius: 4px; background-color: #fafbfc;">'
	});
	...
	analysis_data.mapping_proposals.forEach((proposal) => {
		...
		const display_name
      = proposal.name || proposal.identifier || proposal.category;
		...
		if (proposal.sample_accounts && proposal.sample_accounts.length > 0) {
			...
			const examples = proposal.sample_accounts
				.slice(0, 2)
				.map((acc) => `${acc.code} - ${acc.description || acc.name || ''}`)
				.join(', ');
			row_html += examples;
		...
		primary_action(_values) {
			...
			if (Object.keys(mappings).length === 0) {
				frappe.msgprint({
					...
					message: __(
						'Please select at least one account type mapping before previewing.'
					),
					indicator: 'orange'
				});
				return;
			...
			let preview_method
        = 'verenigingen.e_boekhouden.utils_account_type_mapping.get_mapping_preview';
			...
			if (
				mapping_keys.some(
					(k) =>
						k.includes('_')
            || ['DEB', 'CRED', 'FIN', 'KAS', 'VW', 'BTW', 'EIG', 'BAL'].includes(
            	k
            )
				)
			) {
				preview_method
          = 'verenigingen.e_boekhouden.utils_category_preview.get_category_mapping_preview';
			}
...
function show_mapping_preview(mappings, preview) {
	...
	const preview_dialog = new frappe.ui.Dialog({
		...
		primary_action() {
			...
			let apply_method
        = 'verenigingen.e_boekhouden.utils_account_type_mapping.apply_account_type_mappings';
            ...
            || [
				...
				apply_method
          = 'verenigingen.utils.apply_category_mapping_fixed.apply_fixed_category_mappings';
			}
				...
				callback(r) {
					if (r.message && r.message.success) {
						...
						frappe.msgprint({
							...
							message:
                __(`Successfully updated ${summary.total_updated} accounts.
