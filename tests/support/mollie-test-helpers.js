...
class MollieTestHelpers {
	constructor(page) {
		this.page = page;
		this.testApiKey = process.env.MOLLIE_TEST_API_KEY || 'test_dHar4XY7LxsDOtmnkVtjNVWXLSlXsM';
   ...
   */
	async completeTestPayment(options = {}) {
		...
		console.log(`[Mollie] Starting test payment: ${paymentMethod}, €${amount}, scenario: ${testScenario}`);
		...
		try {
			...
			if (await this.page.locator(`[data-testid="${paymentMethod}"]`).isVisible()) {
			...
			const paymentResult = await this.handlePaymentMethodFlow(paymentMethod, testScenario);
			...
			console.log(`[Mollie] Payment completed with status: ${paymentResult.status}`);
   ...
   */
	async processIdealPayment(testScenario) {
		...
		if (await this.page.locator('[name="issuer"]').isVisible()) {
			...
			const testBank = testScenario === 'success' ? 'ideal_TESTNL99' : 'ideal_TESTNL01';
		...
		if (await this.page.url().includes('testbank') || await this.page.locator('.test-bank-page').isVisible()) {
   ...
   */
	async processCreditCardPayment(testScenario) {
		...
		if (await this.page.locator('[name="cardNumber"]').isVisible()) {
			const testCard = testScenario === 'success' ? '4242424242424242' : '4000000000000002';
			...
			console.log(`[Mollie] Filled credit card details for ${testScenario} scenario`);
		}
   ...
   */
	async handleTestBankFlow(testScenario) {
		console.log(`[Mollie] Handling test bank flow for scenario: ${testScenario}`);

		// Wait for test bank page to load
		await this.page.waitForLoadState('networkidle');

		// Look for test bank buttons/options
		const successButton = this.page.locator('[data-testid="success"], .btn-success, [value="success"]');
		const failButton = this.page.locator('[data-testid="fail"], .btn-danger, [value="fail"]');
   ...
   */
	async handlePayPalSandbox(testScenario) {
		console.log(`[Mollie] Handling PayPal sandbox for scenario: ${testScenario}`);
		...
		if (testScenario === 'success') {
			const approveButton = this.page.locator('#confirmButtonTop, #payment-submit-btn, [data-testid="approve"]');
			if (await approveButton.isVisible()) {
				await approveButton.click();
			}
		} else {
			const cancelButton = this.page.locator('#cancelLink, [data-testid="cancel"]');
	...
	async waitForPaymentProcessing(testScenario, timeout = 15000) {
		...
		try {
			...
			await Promise.race([
				...
				this.page.waitForSelector('.payment-success, .payment-failed, .payment-complete', { timeout }),
   ...
   */
	async waitForScenarioCompletion(testScenario, timeout) {
		switch (testScenario) {
			case 'success':
				return this.page.waitForSelector('.success, .paid, .completed', { timeout });

			case 'failed':
				return this.page.waitForSelector('.failed, .error, .declined', { timeout });
   ...
   */
	generatePaymentResult(testScenario) {
		const scenario = this.testScenarios[testScenario] || this.testScenarios.success;
   ...
   */
	async validateMolliePaymentPage() {
		...
		const validationChecks = [
			{ selector: '.payment-form, .mollie-form', description: 'Payment form present' },
			{ selector: '.amount, .payment-amount', description: 'Payment amount displayed' },
			{ selector: '.payment-methods, .method-selector', description: 'Payment methods available' }
		];
		...
		for (const check of validationChecks) {
			try {
				...
				console.log(`[Mollie] ${check.description}: ${isVisible ? 'PASS' : 'FAIL'}`);
			} catch (error) {
	...
	async getCurrentPaymentStatus() {
		try {
			...
			if (await this.page.locator('.payment-success, .paid').isVisible()) {
				return 'paid';
			} else if (await this.page.locator('.payment-failed, .failed').isVisible()) {
				return 'failed';
			} else if (await this.page.locator('.payment-cancelled, .cancelled').isVisible()) {
				return 'cancelled';
			} else if (await this.page.locator('.payment-pending, .pending').isVisible()) {
				return 'pending';
			}

			return 'unknown';
		} catch (error) {
			console.warn(`[Mollie] Could not determine payment status: ${error.message}`);
			return 'error';
		}
	}
