...
class WebhookSimulator {
  constructor(page) {
    ...
    this.testWebhookSecret =
      process.env.MOLLIE_WEBHOOK_SECRET || "test_webhook_secret_key";
  ...
  generateWebhookSignature(payload, secret = null) {
    ...
    console.log(
      `[Webhook] Generated signature for payload length: ${payload.length}`,
    );
    return signature;
  }
  ...
  async sendMollieWebhook(webhookData) {
    ...
    console.log(
      `[Webhook] Sending Mollie webhook: ${resource} ${paymentId} (${status})`,
    );
    ...
    try {
      ...
      console.log(
        `[Webhook] Webhook response: ${webhookResult.status} ${webhookResult.statusText}`,
      );
  ...
  buildWebhookPayload(params) {
    const template =
      this.webhookTemplates[`${params.resource}_${params.status}`] ||
  ...
  processWebhookResult(webhookResult, originalPayload) {
    ...
    if (!success) {
      result.errors.push(
        `HTTP ${webhookResult.status}: ${webhookResult.statusText}`,
      );
    }

    console.log(
      `[Webhook] Processing result: ${success ? "SUCCESS" : "FAILED"}`,
    );
    return result;
  }
  ...
  async sendWebhookSequence(webhookSequence) {
    console.log(
      `[Webhook] Sending sequence of ${webhookSequence.length} webhooks`,
    );
    ...
    for (let i = 0; i < webhookSequence.length; i++) {
      ...
      console.log(
        `[Webhook] Sending webhook ${i + 1}/${webhookSequence.length}`,
      );
      ...
      try {
        ...
        if (webhookConfig.delay) {
          await new Promise((resolve) =>
            setTimeout(resolve, webhookConfig.delay),
          );
        }
      } catch (error) {
        console.error(
          `[Webhook] Sequence webhook ${i + 1} failed: ${error.message}`,
        );
        results.push({
          processed: false,
          error: error.message,
    ...
    console.log(
      `[Webhook] Sequence completed: ${results.filter((r) => r.processed).length}/${results.length} successful`,
    );
    return results;
  }
  ...
  async testWebhookRateLimiting(requestCount = 10) {
    console.log(
      `[Webhook] Testing rate limiting with ${requestCount} rapid requests`,
    );
    ...
    console.log(
      `[Webhook] Rate limiting test completed: ${summary.rateLimitedRequests}/${summary.totalRequests} rate limited`,
    );
    return summary;
  }
  ...
  async simulateWebhookProcessingDelay(options = {}) {
    ...
    console.log(
      `[Webhook] Simulating webhook processing delay: ${simulatedProcessingTime}ms`,
    );
  ...
  async runComprehensiveWebhookTests() {
    ...
    try {
      ...
      testSuite.signatureVerification =
      ...
      testSuite.sequenceProcessing = await this.sendWebhookSequence([
        { paymentId: "tr_seq_1", status: "pending", amount: 10.0 },
        { paymentId: "tr_seq_2", status: "paid", amount: 20.0, delay: 1000 },
        { paymentId: "tr_seq_3", status: "failed", amount: 15.0, delay: 500 },
      ]);
      ...
      console.log(
        `[Webhook] Comprehensive test suite completed in ${testSuite.completionTime}ms`,
      );
    } catch (error) {
