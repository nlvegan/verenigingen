...
async function globalSetup(config) {
  console.log(
    "[Global Setup] Starting Playwright E2E test environment setup...",
  );
  ...
  try {
    ...
    try {
      ...
      console.warn(
        `[Global Setup] Mollie configuration issue: ${error.message}`,
      );
      console.warn(
        "[Global Setup] Continuing with limited Mollie testing capabilities",
      );
    }
...
async function verifyMollieConfiguration(page) {
  try {
    ...
    if (!mollieConfig.configured) {
      throw new Error(
        "Mollie Settings not found - please configure Mollie integration",
      );
    }

    if (!mollieConfig.testMode) {
      console.warn(
        "[Global Setup] WARNING: Mollie not in test mode - tests may affect production data",
      );
    }
...
async function validateWebhookEndpoints(page) {
  ...
  for (const endpoint of webhookEndpoints) {
    try {
      ...
      throw new Error(
        `Webhook endpoint validation failed for ${endpoint}: ${error.message}`,
      );
    }
  }
