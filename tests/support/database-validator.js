...
class DatabaseValidator {
  constructor(page) {
    ...
    this.donationFields = [
      'donor', 'donor_email', 'donation_date', 'amount', 'mode_of_payment',
      'paid', 'payment_id', 'payment_status', 'bank_reference', 'status',
      'mollie_customer_id', 'mollie_subscription_id', 'mollie_mandate_id',
      'sales_invoice', 'recurring_frequency', 'next_collection_date'
    ];
    
    this.donorFields = [
      'donor_name', 'donor_email', 'phone', 'address_line_1', 'city',
      'postal_code', 'country', 'donor_type', 'communication_preference'
    ];
    
    this.paymentEntryFields = [
      'payment_type', 'party_type', 'party', 'paid_amount', 'received_amount',
  ...
  async executeQuery(query, params = {}) {
    try {
      const result = await this.page.evaluate(async ({ query, params }) => {
  ...
  async verifyDonorExists(criteria) {
    ...
    if (criteria.firstName && criteria.lastName) {
      filters.donor_name = ['like', `%${criteria.firstName}%${criteria.lastName}%`];
    }
    ...
    if (donors.length === 0) {
      console.warn(`[DB] No donor found matching criteria: ${JSON.stringify(criteria)}`);
      return null;
    }
    ...
    if (criteria.email && donor.donor_email !== criteria.email) {
      throw new Error(`Donor email mismatch: expected ${criteria.email}, got ${donor.donor_email}`);
    }
  ...
  async verifyDonationExists(criteria) {
    ...
    if (donations.length === 0) {
      console.warn(`[DB] No donation found matching criteria: ${JSON.stringify(criteria)}`);
      return null;
    }
    
    const donation = donations[0];
    console.log(`[DB] Found donation: ${donation.name} - €${donation.amount} (${donation.status})`);
  ...
  validateDonationFields(donation, criteria) {
    // Amount validation
    if (criteria.amount && parseFloat(donation.amount) !== parseFloat(criteria.amount)) {
      throw new Error(`Donation amount mismatch: expected ${criteria.amount}, got ${donation.amount}`);
    }
    
    // Status validation
    if (criteria.status && donation.status !== criteria.status) {
      throw new Error(`Donation status mismatch: expected ${criteria.status}, got ${donation.status}`);
    }
    ...
    if (criteria.requiresMollie) {
      ...
      if (donation.status === 'Recurring' && !donation.mollie_subscription_id) {
        throw new Error('Missing required Mollie Subscription ID for recurring donation');
      }
    }
  ...
  async verifyPaymentEntryExists(criteria) {
    console.log(`[DB] Verifying payment entry exists: ${JSON.stringify(criteria)}`);
    ...
    if (paymentEntries.length === 0) {
      console.warn(`[DB] No payment entry found matching criteria: ${JSON.stringify(criteria)}`);
      return null;
    }
    
    const paymentEntry = paymentEntries[0];
    console.log(`[DB] Found payment entry: ${paymentEntry.name} - €${paymentEntry.paid_amount}`);
    
    // Track for cleanup
    this.testRecords.add({ doctype: 'Payment Entry', name: paymentEntry.name });
    
    // Validate payment entry fields
    if (criteria.paidAmount && parseFloat(paymentEntry.paid_amount) !== parseFloat(criteria.paidAmount)) {
      throw new Error(`Payment amount mismatch: expected ${criteria.paidAmount}, got ${paymentEntry.paid_amount}`);
    }
  ...
  async verifyPaymentHistoryExists(criteria) {
    console.log(`[DB] Verifying payment history exists: ${JSON.stringify(criteria)}`);
    ...
    if (paymentHistory.length === 0) {
      console.warn(`[DB] No payment history found matching criteria: ${JSON.stringify(criteria)}`);
      return null;
    }
    
    const history = paymentHistory[0];
    console.log(`[DB] Found payment history: ${history.name} - €${history.amount}`);
    
    // Track for cleanup
    this.testRecords.add({ doctype: 'Member Payment History', name: history.name });
  ...
  async verifyWebhookProcessingLogExists(criteria) {
    console.log(`[DB] Verifying webhook processing log exists: ${JSON.stringify(criteria)}`);
    ...
    if (logs.length === 0) {
      console.warn(`[DB] No webhook processing log found matching criteria: ${JSON.stringify(criteria)}`);
      return null;
    }
    
    const log = logs[0];
    console.log(`[DB] Found webhook processing log: ${log.name} (${log.status})`);
  ...
  async verifySalesInvoiceExists(criteria) {
    console.log(`[DB] Verifying sales invoice exists: ${JSON.stringify(criteria)}`);
    ...
    if (invoices.length === 0) {
      console.warn(`[DB] No sales invoice found matching criteria: ${JSON.stringify(criteria)}`);
      return null;
    }
    
    const invoice = invoices[0];
    console.log(`[DB] Found sales invoice: ${invoice.name} - €${invoice.grand_total}`);
  ...
  async verifyCompleteDonationChain(donationData) {
    ...
    try {
      ...
      results.donation = await this.verifyDonationExists({
        ...
        status: donationData.donationType === 'recurring' ? 'Recurring' : 'One-time',
        requiresMollie: true
      });
      ...
      console.log(`[DB] Complete donation chain validation completed with ${results.validationErrors.length} errors`);
      
      return results;
      
    } catch (error) {
      console.error(`[DB] Complete donation chain validation failed: ${error.message}`);
  ...
  async cleanupTestRecords(recordList = null) {
    ...
    for (const record of recordsToCleanup) {
      try {
        ...
        console.warn(`[DB] Failed to cleanup ${record.doctype} ${record.name}: ${error.message}`);
      }
    }
  ...
  async getDatabaseStats() {
    try {
      const stats = await this.page.evaluate(async () => {
        const doctypes = ['Donor', 'Donation', 'Payment Entry', 'Member Payment History'];
        const results = {};
