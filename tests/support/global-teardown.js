...
async function cleanupTestData(page) {
  try {
      ...
      try {
        ...
        for (const donor of testDonors.message || []) {
          try {
            ...
            results.errors.push(
              `Failed to delete Donor ${donor.name}: ${e.message}`,
            );
          }
        }
      } catch (e) {
      ...
      try {
        ...
        for (const donation of testDonations.message || []) {
          try {
            ...
            results.errors.push(
              `Failed to delete Donation ${donation.name}: ${e.message}`,
            );
          }
        }
      } catch (e) {
      ...
      try {
        ...
        for (const payment of testPayments.message || []) {
          try {
            ...
            results.errors.push(
              `Failed to delete Payment Entry ${payment.name}: ${e.message}`,
            );
          }
        }
      } catch (e) {
      ...
      try {
        ...
        for (const log of testLogs.message || []) {
          try {
            ...
            results.errors.push(
              `Failed to delete Webhook Log ${log.name}: ${e.message}`,
            );
          }
        }
      } catch (e) {
    ...
    console.log(
      `  - Payment Entries cleaned: ${cleanupResults.paymentEntries}`,
    );
    console.log(`  - Webhook Logs cleaned: ${cleanupResults.webhookLogs}`);

    if (cleanupResults.errors.length > 0) {
      console.warn(
        `[Global Teardown] ${cleanupResults.errors.length} cleanup errors:`,
      );
...
async function generateTestSummary() {
  try {
    ...
    const summary = {
      ...
      metrics: {
        averageTestDuration: testResults.stats?.duration
          ? testResults.stats.duration / testResults.stats.total
          : 0,
        successRate: testResults.stats?.total
          ? `${((testResults.stats.passed / testResults.stats.total) * 100).toFixed(2)}%`
          : "0%",
      },
    ...
    console.error(
      `[Global Teardown] Failed to generate test summary: ${error.message}`,
    );
  }
}
...
async function archiveTestArtifacts() {
  try {
    ...
    for (const dirName of artifactDirs) {
      ...
      if (fs.existsSync(sourceDir)) {
        ...
        try {
          ...
          console.warn(
            `[Global Teardown] Failed to archive ${dirName}: ${e.message}`,
          );
        }
      }
    }
    ...
    console.log(
      `[Global Teardown] Archived ${archivedCount} test artifacts to: ${archiveSubDir}`,
    );
  } catch (error) {
    console.error(
      `[Global Teardown] Failed to archive test artifacts: ${error.message}`,
    );
  }
}
...
async function performDataIntegrityCheck(page) {
  try {
    ...
    if (integrityCheck.residualTestData > 0) {
      console.warn(
        `[Global Teardown] WARNING: ${integrityCheck.residualTestData} test records remain in database`,
      );
    } else {
    ...
    console.error(
      `[Global Teardown] Data integrity check failed: ${error.message}`,
    );
  }
}
