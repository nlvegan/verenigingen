...
test.describe("Mollie Donation Flow E2E", () => {
  ...
  test("Complete recurring donation flow - Happy Path", async ({ page }) => {
    ...
    await test.step("Fill donation form with recurring donation", async () => {
      ...
      await expect(page.locator('[name="donor_email"]')).toHaveValue(
        testData.email,
      );
      await expect(page.locator('[name="donation_amount"]')).toHaveValue(
        "25.00",
      );
    ...
    await test.step("Submit donation form and verify Mollie redirect", async () => {
      ...
      const navigationPromise = page.waitForURL(
        /mollie\.com|payments\.mollie\.com/,
      );
   ...
   */
  test("Single donation flow with different payment method", async ({
    page,
  }) => {
   ...
   */
  test("Error scenarios - Payment failures and validation", async ({
    page,
  }) => {
   ...
   */
  test("Performance validation - Multiple concurrent donations", async ({
    page,
    browser,
  }) => {
    ...
    await test.step("Process multiple donations concurrently", async () => {
      ...
      for (let i = 0; i < concurrentDonations; i++) {
        ...
        const donationPromise = (async () => {
          ...
          await testPage.fill(
            ...
            perfTestData.firstName,
          );
          await testPage.fill(
            '[name="donor_last_name"]',
            perfTestData.lastName,
          );
      ...
      console.log(
        `[E2E] ${results.length} concurrent donations processed successfully`,
      );
   ...
   */
  test("Webhook processing validation with detailed logging", async ({
    page,
  }) => {
    test.setTimeout(90000);
    ...
    await test.step("Test webhook processing with logging validation", async () => {
      ...
      console.log(
        `[E2E] Webhook processing log verified: ${processingLog.name}`,
      );
    });
    ...
    await test.step("Verify comprehensive database updates", async () => {
      ...
      console.log(
        "[E2E] Comprehensive database validation completed successfully",
      );
    });
  });
});
