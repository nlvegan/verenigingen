...
module.exports = {
	...
	create(context) {
		const dangerousMethods = ['html', 'innerHTML', 'append', 'prepend', 'after', 'before', 'replaceWith'];

		const _safeMethods = ['text', 'val', 'attr'];

		return {
			CallExpression(node) {
				// Check jQuery methods
				if (node.callee.type === 'MemberExpression' && dangerousMethods.includes(node.callee.property.name)) {
			...
			AssignmentExpression(node) {
				...
				if (node.left.type === 'MemberExpression' && node.left.property.name === 'innerHTML') {
					context.report({
						node,
						message:
							'Avoid direct innerHTML assignment. Use textContent, frappe.render_template(), or proper sanitization.'
					});
				}
			}
		};
