      ...
      CallExpression(node) {
        ...
        if (
          ...
          if (args.length >= 3) {
            ...
            if (
              fieldsArg.type === "Literal" &&
              typeof fieldsArg.value === "string"
            ) {
              // Single field is OK, but recommend array for consistency
              context.report({
                node,
                message:
                  "Consider using array format for fields parameter in frappe.db.get_value() for consistency, even for single fields",
              });
            }
          }
        }
        ...
        if (
          ...
          if (args.length > 0 && args[0].type === "ObjectExpression") {
            const methodProp = args[0].properties.find(
              ...
              if (typeof methodPath === "string" && methodPath.includes(".")) {
                ...
                if (parts.length < 3) {
                  context.report({
                    node: methodProp,
                    message:
                      'Frappe method calls should use full module path (e.g., "app.module.function")',
                  });
                }
              }
            }
        ...
        if (
          ...
          if (args.length >= 2) {
            ...
            if (
              fieldArg.type === "Literal" &&
              typeof fieldArg.value === "string"
            ) {
              ...
              if (fieldName.includes("_") && fieldName.endsWith("_id")) {
                context.report({
                  node: fieldArg,
                  message:
                    "Verify that field name exists in DocType definition. Consider using field validator.",
                });
              }
            }
          }
        ...
        if (
          node.callee.type === "MemberExpression" &&
          node.callee.object.name === "frm" &&
          ["save", "reload_doc", "submit", "cancel"].includes(
