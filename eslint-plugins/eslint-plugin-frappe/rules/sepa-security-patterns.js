...
module.exports = {
	...
	create(context) {
		...
		return {
			...
			CallExpression(node) {
				...
				if (node.callee.type === 'Identifier' && ['alert', 'msgprint'].includes(node.callee.name)) {
					node.arguments.forEach((arg) => {
						if (this.containsSensitiveData(arg)) {
							context.report({
								node: arg,
	...
	containsSensitiveData(node) {
		...
		if (node.type === 'Identifier') {
			return sensitivePatterns.some((pattern) => node.name.toLowerCase().includes(pattern));
		}

		// Check member expressions (obj.prop)
		if (node.type === 'MemberExpression' && node.property.type === 'Identifier') {
			return sensitivePatterns.some((pattern) => node.property.name.toLowerCase().includes(pattern));
		}
