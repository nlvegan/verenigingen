...
module.exports = {
    ...
    return {
          ...
          node.arguments.forEach((arg) => {
            if (this.containsSensitiveData(arg)) {
              context.report({
                node: arg,
                message:
                  "Avoid logging sensitive financial data (IBAN, BIC, account numbers, etc.)",
              });
            }
          });
        }
          ...
          node.arguments.forEach((arg) => {
            if (this.containsSensitiveData(arg)) {
              context.report({
                node: arg,
                message:
                  "Avoid displaying sensitive financial data in alerts or messages",
              });
            }
          });
        }
        ...
        if (
          ...
          if (
            ...
            sensitiveFields.some((field) =>
              firstArg.name.toLowerCase().includes(field),
            )
          ) {
            context.report({
              node,
              message:
                "Ensure IBAN/BIC validation includes proper sanitization and format checking",
            });
          }
        }
      },
      ...
      MemberExpression(node) {
        if (
          ...
          if (
            sensitiveFields.some((sensitive) => fieldName.includes(sensitive))
          ) {
          ...
          [node.left, node.right].forEach((operand) => {
            if (this.containsSensitiveData(operand)) {
              context.report({
                node: operand,
                message:
                  "Avoid string concatenation with sensitive financial data. Use secure templating instead.",
              });
            }
          });
        }
        ...
        node.expressions.forEach((expr) => {
          if (this.containsSensitiveData(expr)) {
            context.report({
              node: expr,
              message:
                "Avoid including sensitive financial data in template literals. Consider masking or sanitization.",
            });
          }
        });
      },
  ...
  containsSensitiveData(node) {
    ...
    if (node.type === "Literal" && typeof node.value === "string") {
      ...
      return (
        sensitivePatterns.some((pattern) => value.includes(pattern)) ||
        /\b[a-z]{2}\d{2}[a-z0-9]{4,30}\b/i.test(node.value) || // IBAN pattern
        /\b[a-z]{4}[a-z]{2}[a-z0-9]{2}([a-z0-9]{3})?\b/i.test(node.value)
      ); // BIC pattern
    }
