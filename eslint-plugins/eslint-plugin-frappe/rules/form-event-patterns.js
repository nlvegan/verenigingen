	...
	create(context) {
		return {
			...
			CallExpression(node) {
						...
						if (handlersArg.type === 'ObjectExpression') {
							handlersArg.properties.forEach((prop) => {
								if (prop.key && prop.value.type === 'FunctionExpression') {
									...
									if (
										['refresh', 'onload', 'validate', 'before_save', 'after_save'].includes(
											eventName
										)
									) {
				...
				if (node.callee.type === 'Identifier' && node.callee.name === 'validate') {
			...
			MemberExpression(node) {
				if (node.object.name === 'frm' && node.property.name === 'add_custom_button') {
