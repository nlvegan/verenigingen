      ...
      CallExpression(node) {
        if (
          ...
          if (args.length >= 2) {
            ...
            if (
              docTypeArg.type !== "Literal" ||
              typeof docTypeArg.value !== "string"
            ) {
              context.report({
                node: docTypeArg,
                message:
                  "DocType name should be a string literal in frappe.ui.form.on()",
              });
            }
            ...
            if (handlersArg.type === "ObjectExpression") {
              handlersArg.properties.forEach((prop) => {
                if (prop.key && prop.value.type === "FunctionExpression") {
                  ...
                  if (
                    eventName.includes("_add") ||
                    eventName.includes("_remove")
                  ) {
                    if (func.params.length < 3) {
                      ...
                      if (
                        ...
                        context.report({
                          node: func,
                          message:
                            "Child table event parameters should be named (frm, cdt, cdn)",
                        });
                      }
                    }
                  }
        ...
        if (
          ...
          context.report({
            node,
            message:
              "Validate functions should return false to prevent save, or true/undefined to allow",
          });
        }
      },
        ...
        if (
          ...
          if (parent && parent.type === "CallExpression") {
            ...
            if (args.length >= 2) {
              ...
              if (
                labelArg.type === "Literal" &&
                typeof labelArg.value === "string"
              ) {
                if (!labelArg.value.startsWith("__")) {
                  context.report({
                    node: labelArg,
                    message:
                      "Button labels should be wrapped with __(label) for translation",
                  });
                }
              }
